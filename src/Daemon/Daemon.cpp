...
namespace
{
  const command_line::arg_descriptor<std::string> arg_config_file = {"config-file", "Specify configuration file", std::string(TycheCash::TycheCash_NAME) + ".conf"};
...
void print_genesis_tx_hex() {
  ...
  TycheCash::Transaction tx = TycheCash::CurrencyBuilder(logger).generateGenesisTransaction();
  TycheCash::BinaryArray txb = TycheCash::toBinaryArray(tx);
...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    bool r = command_line::handle_error_helper(desc_options, [&]()
    {
      ...
      {
        std::cout << TycheCash::TycheCash_NAME << " v" << PROJECT_VERSION_LONG << ENDL << ENDL;
    ...
    logger(INFO) << TycheCash::TycheCash_NAME << " v" << PROJECT_VERSION_LONG;
    ...
    TycheCash::CurrencyBuilder currencyBuilder(logManager);
    currencyBuilder.testnet(testnet_mode);

    try {
      currencyBuilder.currency();
    } catch (std::exception&) {
      std::cout << "GENESIS_COINBASE_TX_HEX constant has an incorrect value. Please launch: " << TycheCash::TycheCash_NAME << "d --" << arg_print_genesis_tx.name;
      return 1;
    }

    TycheCash::Currency currency = currencyBuilder.currency();
    TycheCash::core ccore(currency, nullptr, logManager);

    TycheCash::Checkpoints checkpoints(logManager);
    for (const auto& cp : TycheCash::CHECKPOINTS) {
    ...
    TycheCash::TycheCashProtocolHandler cprotocol(currency, dispatcher, ccore, nullptr, logManager);
    TycheCash::NodeServer p2psrv(dispatcher, cprotocol, logManager);
    TycheCash::RpcServer rpcServer(dispatcher, logManager, ccore, p2psrv, cprotocol);

    cprotocol.set_p2p_endpoint(&p2psrv);
    ccore.set_TycheCash_protocol(&cprotocol);
    ...
    ccore.set_TycheCash_protocol(NULL);
...
bool command_line_preprocessor(const boost::program_options::variables_map &vm, LoggerRef &logger) {
  ...
  if (command_line::get_arg(vm, command_line::arg_version)) {
    std::cout << TycheCash::TycheCash_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
    exit = true;
  }

...
namespace {
  ...
  static bool print_as_json(const T& obj) {
    std::cout << TycheCash::storeToJson(obj) << ENDL;
    return true;
  }
}


DaemonCommandsHandler::DaemonCommandsHandler(TycheCash::core& core, TycheCash::NodeServer& srv, Logging::LoggerManager& log) :
...
{
  std::stringstream ss;
  ss << TycheCash::TycheCash_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
  ss << "Commands: " << ENDL;
...
bool DaemonCommandsHandler::start_mining(const std::vector<std::string> &args) {
  ...
  TycheCash::AccountPublicAddress adr;

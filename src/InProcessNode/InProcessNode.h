...
class InProcessNode : public INode, public TycheCash::ITycheCashProtocolObserver, public TycheCash::ICoreObserver {
public:
  InProcessNode(TycheCash::ICore& core, TycheCash::ITycheCashProtocolQuery& protocol);
...
private:
  ...
  void getNewBlocksAsync(std::vector<Crypto::Hash>& knownBlockIds, std::vector<TycheCash::block_complete_entry>& newBlocks, uint32_t& startHeight, const Callback& callback);
  std::error_code doGetNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds, std::vector<TycheCash::block_complete_entry>& newBlocks, uint32_t& startHeight);
  ...
  std::error_code doGetRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint64_t outsCount,
      std::vector<TycheCash::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result);

  void relayTransactionAsync(const TycheCash::Transaction& transaction, const Callback& callback);
  std::error_code doRelayTransaction(const TycheCash::Transaction& transaction);

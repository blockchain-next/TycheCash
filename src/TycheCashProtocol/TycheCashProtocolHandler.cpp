...
#include "TycheCashProtocolHandler.h"
...
#include "TycheCashCore/VerificationContext.h"
...
bool post_notify(IP2pEndpoint& p2p, typename t_parametr::request& arg, const TycheCashConnectionContext& context) {
...
TycheCashProtocolHandler::TycheCashProtocolHandler(const Currency& currency, System::Dispatcher& dispatcher, ICore& rcore, IP2pEndpoint* p_net_layout, Logging::ILogger& log) :
...
size_t TycheCashProtocolHandler::getPeerCount() const {
  return m_peersCount;
}

void TycheCashProtocolHandler::set_p2p_endpoint(IP2pEndpoint* p2p) {
  if (p2p)
    m_p2p = p2p;
  else
    m_p2p = &m_p2p_stub;
...
void TycheCashProtocolHandler::onConnectionOpened(TycheCashConnectionContext& context) {
}

void TycheCashProtocolHandler::onConnectionClosed(TycheCashConnectionContext& context) {
  bool updated = false;
  {
  ...
  if (updated) {
    ...
    m_observerManager.notify(&ITycheCashProtocolObserver::lastKnownBlockHeightUpdated, m_observedHeight);
  }

  if (context.m_state != TycheCashConnectionContext::state_befor_handshake) {
    m_peersCount--;
    m_observerManager.notify(&ITycheCashProtocolObserver::peerCountUpdated, m_peersCount.load());
  }
}

void TycheCashProtocolHandler::stop() {
  m_stop = true;
}

bool TycheCashProtocolHandler::start_sync(TycheCashConnectionContext& context) {
  logger(Logging::TRACE) << context << "Starting synchronization";

  if (context.m_state == TycheCashConnectionContext::state_synchronizing) {
...
bool TycheCashProtocolHandler::get_stat_info(core_stat_info& stat_inf) {
  return m_core.get_stat_info(stat_inf);
}

void TycheCashProtocolHandler::log_connections() {
  std::stringstream ss;
  ...
  m_p2p->for_each_connection([&](const TycheCashConnectionContext& cntxt, PeerIdType peer_id) {
...
uint32_t TycheCashProtocolHandler::get_current_blockchain_height() {
  uint32_t height;
  Crypto::Hash blockId;
...
bool TycheCashProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA& hshd, TycheCashConnectionContext& context, bool is_inital) {
  if (context.m_state == TycheCashConnectionContext::state_befor_handshake && !is_inital)
    return true;
  ...
  if (context.m_state == TycheCashConnectionContext::state_synchronizing) {
    ...
    if (is_inital) {
      ...
      context.m_state = TycheCashConnectionContext::state_pool_sync_required;
    } else {
      context.m_state = TycheCashConnectionContext::state_normal;
    }
  } else {
    ...
    context.m_state = TycheCashConnectionContext::state_sync_required;
  }
...
bool TycheCashProtocolHandler::get_payload_sync_data(CORE_SYNC_DATA& hshd) {
...
int notifyAdaptor(const BinaryArray& reqBuf, TycheCashConnectionContext& ctx, Handler handler) {
...
int TycheCashProtocolHandler::handleCommand(bool is_notify, int command, const BinaryArray& in, BinaryArray& out, TycheCashConnectionContext& ctx, bool& handled) {
  int ret = 0;
  handled = true;

  switch (command) {
    HANDLE_NOTIFY(NOTIFY_NEW_BLOCK, &TycheCashProtocolHandler::handle_notify_new_block)
    HANDLE_NOTIFY(NOTIFY_NEW_TRANSACTIONS, &TycheCashProtocolHandler::handle_notify_new_transactions)
    HANDLE_NOTIFY(NOTIFY_REQUEST_GET_OBJECTS, &TycheCashProtocolHandler::handle_request_get_objects)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_GET_OBJECTS, &TycheCashProtocolHandler::handle_response_get_objects)
    HANDLE_NOTIFY(NOTIFY_REQUEST_CHAIN, &TycheCashProtocolHandler::handle_request_chain)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_CHAIN_ENTRY, &TycheCashProtocolHandler::handle_response_chain_entry)
    HANDLE_NOTIFY(NOTIFY_REQUEST_TX_POOL, &TycheCashProtocolHandler::handleRequestTxPool)
...
int TycheCashProtocolHandler::handle_notify_new_block(int command, NOTIFY_NEW_BLOCK::request& arg, TycheCashConnectionContext& context) {
  ...
  if (context.m_state != TycheCashConnectionContext::state_normal) {
    return 1;
  }

  for (auto tx_blob_it = arg.b.txs.begin(); tx_blob_it != arg.b.txs.end(); tx_blob_it++) {
    TycheCash::tx_verification_context tvc = boost::value_initialized<decltype(tvc)>();
    ...
    if (tvc.m_verification_failed) {
      ...
      context.m_state = TycheCashConnectionContext::state_shutdown;
      return 1;
    }
  }
  ...
  if (bvc.m_added_to_main_chain) {
    ...
    context.m_state = TycheCashConnectionContext::state_synchronizing;
    NOTIFY_REQUEST_CHAIN::request r = boost::value_initialized<NOTIFY_REQUEST_CHAIN::request>();
    r.block_ids = m_core.buildSparseChain();
    logger(Logging::TRACE) << context << "-->>NOTIFY_REQUEST_CHAIN: m_block_ids.size()=" << r.block_ids.size();
    post_notify<NOTIFY_REQUEST_CHAIN>(*m_p2p, r, context);
...
int TycheCashProtocolHandler::handle_notify_new_transactions(int command, NOTIFY_NEW_TRANSACTIONS::request& arg, TycheCashConnectionContext& context) {
...
int TycheCashProtocolHandler::handle_request_get_objects(int command, NOTIFY_REQUEST_GET_OBJECTS::request& arg, TycheCashConnectionContext& context) {
...
int TycheCashProtocolHandler::handle_response_get_objects(int command, NOTIFY_RESPONSE_GET_OBJECTS::request& arg, TycheCashConnectionContext& context) {
  ...
  for (const block_complete_entry& block_entry : arg.blocks) {
    ...
    if (count == 2) {
      if (m_core.have_block(get_block_hash(b))) {
        context.m_state = TycheCashConnectionContext::state_idle;
  ...
  if (!m_stop && context.m_state == TycheCashConnectionContext::state_synchronizing) {
...
int TycheCashProtocolHandler::processObjects(TycheCashConnectionContext& context, const std::vector<block_complete_entry>& blocks) {
...
bool TycheCashProtocolHandler::on_idle() {
  return m_core.on_idle();
}

int TycheCashProtocolHandler::handle_request_chain(int command, NOTIFY_REQUEST_CHAIN::request& arg, TycheCashConnectionContext& context) {
...
bool TycheCashProtocolHandler::request_missing_objects(TycheCashConnectionContext& context, bool check_having_blocks) {
...
bool TycheCashProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    ...
    m_observerManager.notify(&ITycheCashProtocolObserver::blockchainSynchronized, height);
  }
  return true;
}

int TycheCashProtocolHandler::handle_response_chain_entry(int command, NOTIFY_RESPONSE_CHAIN_ENTRY::request& arg, TycheCashConnectionContext& context) {
...
int TycheCashProtocolHandler::handleRequestTxPool(int command, NOTIFY_REQUEST_TX_POOL::request& arg,
                                                     TycheCashConnectionContext& context) {
...
void TycheCashProtocolHandler::relay_block(NOTIFY_NEW_BLOCK::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_BLOCK::ID, buf);
}

void TycheCashProtocolHandler::relay_transactions(NOTIFY_NEW_TRANSACTIONS::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_TRANSACTIONS::ID, buf);
}

void TycheCashProtocolHandler::requestMissingPoolTransactions(const TycheCashConnectionContext& context) {
...
void TycheCashProtocolHandler::updateObservedHeight(uint32_t peerHeight, const TycheCashConnectionContext& context) {
  bool updated = false;
  {
    std::lock_guard<std::mutex> lock(m_observedHeightMutex);
...
void TycheCashProtocolHandler::recalculateMaxObservedHeight(const TycheCashConnectionContext& context) {
  //should be locked outside
  uint32_t peerHeight = 0;
  m_p2p->for_each_connection([&peerHeight, &context](const TycheCashConnectionContext& ctx, PeerIdType peerId) {
...
uint32_t TycheCashProtocolHandler::getObservedHeight() const {
  std::lock_guard<std::mutex> lock(m_observedHeightMutex);
  return m_observedHeight;
};

bool TycheCashProtocolHandler::addObserver(ITycheCashProtocolObserver* observer) {
  return m_observerManager.add(observer);
}

bool TycheCashProtocolHandler::removeObserver(ITycheCashProtocolObserver* observer) {

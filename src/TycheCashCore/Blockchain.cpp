...
bool serialize(google::sparse_hash_map<K, V, Hash>& value, Common::StringView name, TycheCash::ISerializer& serializer) {
  return serializeMap(value, name, serializer, [&value](size_t size) { value.resize(size); });
}

template<typename K, typename Hash>
bool serialize(google::sparse_hash_set<K, Hash>& value, Common::StringView name, TycheCash::ISerializer& serializer) {
...
bool serialize(std::vector<std::pair<Blockchain::TransactionIndex, uint16_t>>& value, Common::StringView name, TycheCash::ISerializer& s) {
  ...
  if (s.type() == TycheCash::ISerializer::INPUT) {
...
bool Blockchain::checkTransactionInputs(const TycheCash::Transaction& tx, BlockInfo& maxUsedBlock) {
  return checkTransactionInputs(tx, maxUsedBlock.height, maxUsedBlock.id);
}

bool Blockchain::checkTransactionInputs(const TycheCash::Transaction& tx, BlockInfo& maxUsedBlock, BlockInfo& lastFailed) {
...
bool Blockchain::haveSpentKeyImages(const TycheCash::Transaction& tx) {
...
bool Blockchain::validate_miner_transaction(const Block& b, uint32_t height, size_t cumulativeBlockSize,
  ...
  // TODO: fixme, an issue in the blockchain with rewards...
  if (height <= parameters::TycheCash_HARDFORK_HEIGHT_V2) {
    return true;
  }
...
bool Blockchain::handleGetObjects(NOTIFY_REQUEST_GET_OBJECTS::request& arg, NOTIFY_RESPONSE_GET_OBJECTS::request& rsp) { //Deprecated. Should be removed with TycheCashProtocolHandler.
  std::lock_guard<decltype(m_blockchain_lock)> lk(m_blockchain_lock);
...
bool Blockchain::check_tx_input(const KeyInput& txin, const Crypto::Hash& tx_prefix_hash, const std::vector<Crypto::Signature>& sig, uint32_t* pmax_related_block_height) {
  ...
  if (!Crypto::validateKeyImage(txin.keyImage)) {
    logger(ERROR, BRIGHT_RED) << "Invalid key image: " << txin.keyImage << ", amount: " << std::to_string(txin.amount) << ", tx: " << tx_prefix_hash;
    return false;
  }

...
namespace TycheCash {
  ...
  public:
    ...
    TransactionImpl(const TycheCash::Transaction& tx);
  ...
  std::unique_ptr<ITransaction> createTransaction(const TycheCash::Transaction& tx) {
    return std::unique_ptr<ITransaction>(new TransactionImpl(tx));
  }

  TransactionImpl::TransactionImpl() {   
    TycheCash::KeyPair txKeys(TycheCash::generateKeyPair());
  ...
  TransactionImpl::TransactionImpl(const TycheCash::Transaction& tx) : transaction(tx) {
    extra.parse(transaction.extra);
  }
  ...
  bool TransactionImpl::findOutputsToAccount(const AccountPublicAddress& addr, const SecretKey& viewSecretKey, std::vector<uint32_t>& out, uint64_t& amount) const {
    return ::TycheCash::findOutputsToAccount(transaction, addr, viewSecretKey, out, amount);
  }

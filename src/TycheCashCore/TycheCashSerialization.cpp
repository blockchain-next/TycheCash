...
struct BinaryVariantTagGetter: boost::static_visitor<uint8_t> {
  uint8_t operator()(const TycheCash::BaseInput) { return  0xff; }
  uint8_t operator()(const TycheCash::KeyInput) { return  0x2; }
  uint8_t operator()(const TycheCash::MultisignatureInput) { return  0x3; }
  uint8_t operator()(const TycheCash::KeyOutput) { return  0x2; }
  uint8_t operator()(const TycheCash::MultisignatureOutput) { return  0x3; }
  uint8_t operator()(const TycheCash::Transaction) { return  0xcc; }
  uint8_t operator()(const TycheCash::Block) { return  0xbb; }
};

struct VariantSerializer : boost::static_visitor<> {
  VariantSerializer(TycheCash::ISerializer& serializer, const std::string& name) : s(serializer), name(name) {}

  template <typename T>
  void operator() (T& param) { s(param, name); }

  TycheCash::ISerializer& s;
  std::string name;
};

void getVariantValue(TycheCash::ISerializer& serializer, uint8_t tag, TycheCash::TransactionInput& in) {
  switch(tag) {
  case 0xff: {
    TycheCash::BaseInput v;
    serializer(v, "value");
    in = v;
    break;
  }
  case 0x2: {
    TycheCash::KeyInput v;
    serializer(v, "value");
    in = v;
    break;
  }
  case 0x3: {
    TycheCash::MultisignatureInput v;
    serializer(v, "value");
    in = v;
    break;
  }
...
void getVariantValue(TycheCash::ISerializer& serializer, uint8_t tag, TycheCash::TransactionOutputTarget& out) {
  switch(tag) {
  case 0x2: {
    TycheCash::KeyOutput v;
    serializer(v, "data");
    out = v;
    break;
  }
  case 0x3: {
    TycheCash::MultisignatureOutput v;
    serializer(v, "data");
    out = v;
    break;
  }
...
template <typename T>
bool serializePod(T& v, Common::StringView name, TycheCash::ISerializer& serializer) {
  return serializer.binary(&v, sizeof(v), name);
}

bool serializeVarintVector(std::vector<uint32_t>& vector, TycheCash::ISerializer& serializer, Common::StringView name) {
...
bool serialize(PublicKey& pubKey, Common::StringView name, TycheCash::ISerializer& serializer) {
  return serializePod(pubKey, name, serializer);
}

bool serialize(SecretKey& secKey, Common::StringView name, TycheCash::ISerializer& serializer) {
  return serializePod(secKey, name, serializer);
}

bool serialize(Hash& h, Common::StringView name, TycheCash::ISerializer& serializer) {
  return serializePod(h, name, serializer);
}

bool serialize(KeyImage& keyImage, Common::StringView name, TycheCash::ISerializer& serializer) {
  return serializePod(keyImage, name, serializer);
}

bool serialize(chacha8_iv& chacha, Common::StringView name, TycheCash::ISerializer& serializer) {
  return serializePod(chacha, name, serializer);
}

bool serialize(Signature& sig, Common::StringView name, TycheCash::ISerializer& serializer) {
  return serializePod(sig, name, serializer);
}

bool serialize(EllipticCurveScalar& ecScalar, Common::StringView name, TycheCash::ISerializer& serializer) {
  return serializePod(ecScalar, name, serializer);
}

bool serialize(EllipticCurvePoint& ecPoint, Common::StringView name, TycheCash::ISerializer& serializer) {

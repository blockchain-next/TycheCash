...
core::core(const Currency& currency, i_TycheCash_protocol* pprotocol, Logging::ILogger& logger) :
m_currency(currency),
logger(logger, "core"),
...
m_starter_message_showed(false) {
  set_TycheCash_protocol(pprotocol);
...
void core::set_TycheCash_protocol(i_TycheCash_protocol* pprotocol) {
  if (pprotocol)
...
bool core::handle_incoming_tx(const BinaryArray& tx_blob, tx_verification_context& tvc, bool keeped_by_block) { //Deprecated. Should be removed with TycheCashProtocolHandler.
...
bool core::handle_get_objects(NOTIFY_REQUEST_GET_OBJECTS::request& arg, NOTIFY_RESPONSE_GET_OBJECTS::request& rsp) { //Deprecated. Should be removed with TycheCashProtocolHandler.

...
protected:
  ...
  struct ReceiverAmounts {
    TycheCash::AccountPublicAddress receiver;
  ...
  void updateBalance(TycheCash::ITransfersContainer* container);
  ...
  const WalletRecord& getWalletRecord(TycheCash::ITransfersContainer* container) const;

  TycheCash::AccountPublicAddress parseAddress(const std::string& address) const;
  ...
  void prepareTransaction(std::vector<WalletOuts>&& wallets,
    ...
    const TycheCash::AccountPublicAddress& changeDestinationAddress,
  ...
  std::unique_ptr<TycheCash::ITransaction> makeTransaction(const std::vector<ReceiverAmounts>& decomposedOutputs,
    std::vector<InputInfo>& keysInfo, const std::string& extra, uint64_t unlockTimestamp);

  void sendTransaction(const TycheCash::Transaction& TycheCashTransaction);
  ...
  bool updateWalletTransactionInfo(size_t transactionId, const TycheCash::TransactionInformation& info, int64_t totalAmount);
  ...
  void insertUnlockTransactionJob(const Crypto::Hash& transactionHash, uint32_t blockHeight, TycheCash::ITransfersContainer* container);
  ...
  TycheCash::AccountPublicAddress getChangeDestination(const std::string& changeDestinationAddress, const std::vector<std::string>& sourceAddresses) const;

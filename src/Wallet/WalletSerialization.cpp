...
struct WalletTransactionDto {
  ...
  WalletTransactionDto(const TycheCash::WalletTransaction& wallet) {
    state = wallet.state;
  ...
  TycheCash::WalletTransactionState state;
  uint64_t timestamp;
  uint32_t blockHeight;
  Hash hash;
  int64_t totalAmount;
...
struct WalletTransferDto {
  ...
  WalletTransferDto(const TycheCash::WalletTransfer& tr, uint32_t version) : WalletTransferDto(version) {
    address = tr.address;
    amount = tr.amount;
...
void serialize(WalletRecordDto& value, TycheCash::ISerializer& serializer) {
...
void serialize(ObsoleteSpentOutputDto& value, TycheCash::ISerializer& serializer) {
...
void serialize(ObsoleteChangeDto& value, TycheCash::ISerializer& serializer) {
  serializer(value.txHash, "transaction_hash");
  serializer(value.amount, "amount");
}

void serialize(UnlockTransactionJobDto& value, TycheCash::ISerializer& serializer) {
  serializer(value.blockHeight, "block_height");
  serializer(value.transactionHash, "transaction_hash");
  serializer(value.walletIndex, "wallet_index");
}

void serialize(WalletTransactionDto& value, TycheCash::ISerializer& serializer) {
  typedef std::underlying_type<TycheCash::WalletTransactionState>::type StateType;

  StateType state = static_cast<StateType>(value.state);
  serializer(state, "state");
  value.state = static_cast<TycheCash::WalletTransactionState>(state);

  serializer(value.timestamp, "timestamp");
  TycheCash::serializeBlockHeight(serializer, value.blockHeight, "block_height");
...
void serialize(WalletTransferDto& value, TycheCash::ISerializer& serializer) {
...
std::string encrypt(const std::string& plain, TycheCash::CryptoContext& cryptoContext) {
  std::string cipher;
...
void addToStream(const std::string& cipher, const std::string& name, Common::IOutputStream& destination) {
  TycheCash::BinaryOutputStreamSerializer s(destination);
  s(const_cast<std::string& >(cipher), name);
}

template<typename Object>
void serializeEncrypted(Object& obj, const std::string& name, TycheCash::CryptoContext& cryptoContext, Common::IOutputStream& destination) {
...
std::string readCipher(Common::IInputStream& source, const std::string& name) {
  std::string cipher;
  TycheCash::BinaryInputStreamSerializer s(source);
  s(cipher, name);

  return cipher;
}

std::string decrypt(const std::string& cipher, TycheCash::CryptoContext& cryptoContext) {
  std::string plain;
...
void deserialize(Object& obj, const std::string& name, const std::string& plain) {
  ...
  TycheCash::BinaryInputStreamSerializer s(stream);
  s(obj, Common::StringView(name));
}

template<typename Object>
void deserializeEncrypted(Object& obj, const std::string& name, TycheCash::CryptoContext& cryptoContext, Common::IInputStream& source) {
...
TycheCash::WalletTransaction convert(const TycheCash::WalletLegacyTransaction& tx) {
  TycheCash::WalletTransaction mtx;

  mtx.state = TycheCash::WalletTransactionState::SUCCEEDED;
...
TycheCash::WalletTransfer convert(const TycheCash::WalletLegacyTransfer& tr) {
  TycheCash::WalletTransfer mtr;
...
void WalletSerializer::loadWallet(Common::IInputStream& source, const std::string& password, uint32_t version) {
  TycheCash::CryptoContext cryptoContext;
...
void WalletSerializer::loadWalletV1(Common::IInputStream& source, const std::string& password) {
  ...
  TycheCash::BinaryInputStreamSerializer encrypted(source);
...
void WalletSerializer::loadWalletV1Keys(TycheCash::BinaryInputStreamSerializer& serializer) {
...
void WalletSerializer::loadWalletV1Details(TycheCash::BinaryInputStreamSerializer& serializer) {
...
void WalletSerializer::loadWallets(Common::IInputStream& source, CryptoContext& cryptoContext) {
  ...
  for (uint64_t i = 0; i < count; ++i) {
    ...
    wallet.container = reinterpret_cast<TycheCash::ITransfersContainer*>(i); //dirty hack. container field must be unique

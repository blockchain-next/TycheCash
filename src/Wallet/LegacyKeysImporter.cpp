...
struct keys_file_data {
  ...
  void serialize(TycheCash::ISerializer& s) {
    s(iv, "iv");
...
void loadKeysFromFile(const std::string& filename, const std::string& password, TycheCash::AccountBase& account) {
  keys_file_data keys_file_data;
  std::string buf;

  if (!Common::loadFileToString(filename, buf)) {
    throw std::system_error(make_error_code(TycheCash::error::INTERNAL_WALLET_ERROR), "failed to load \"" + filename + '\"');
  }

  if (!TycheCash::fromBinaryArray(keys_file_data, Common::asBinaryArray(buf))) {
    throw std::system_error(make_error_code(TycheCash::error::INTERNAL_WALLET_ERROR), "failed to deserialize \"" + filename + '\"');
  }
  ...
  const TycheCash::AccountKeys& keys = account.getAccountKeys();

  if (TycheCash::loadFromBinaryKeyValue(account, account_data) &&
...
void importLegacyKeys(const std::string& legacyKeysFilename, const std::string& password, std::ostream& destination) {
  ...
  TycheCash::WalletUserTransactionsCache transactionsCache;
  std::string cache;
  TycheCash::WalletLegacySerializer importer(account, transactionsCache);

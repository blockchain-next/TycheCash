...
std::string get_nix_version_display_string()
{
  ...
  {
    //namespace fs = boost::filesystem;
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TycheCash_NAME
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TycheCash_NAME
    // Mac: ~/Library/Application Support/TycheCash_NAME
    // Unix: ~/.TycheCash_NAME
    std::string config_folder;
#ifdef WIN32
    // Windows
    config_folder = get_special_folder_path(CSIDL_APPDATA, true) + "/" + TycheCash::TycheCash_NAME;
#else
    std::string pathRet;
    ...
    config_folder =  (pathRet + "/" + TycheCash::TycheCash_NAME);
#else
    // Unix
    config_folder = (pathRet + "/." + TycheCash::TycheCash_NAME);
#endif

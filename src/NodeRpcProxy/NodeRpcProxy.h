...
class NodeRpcProxy : public TycheCash::INode {
public:
  ...
  virtual bool addObserver(TycheCash::INodeRpcProxyObserver* observer);
  virtual bool removeObserver(TycheCash::INodeRpcProxyObserver* observer);
...
private:
  ...
  std::error_code doGetNewBlocks(std::vector<Crypto::Hash>& knownBlockIds,
    std::vector<TycheCash::block_complete_entry>& newBlocks, uint32_t& startHeight);
...
private:
  ...
  Tools::ObserverManager<TycheCash::INodeObserver> m_observerManager;
  Tools::ObserverManager<TycheCash::INodeRpcProxyObserver> m_rpcProxyObserverManager;

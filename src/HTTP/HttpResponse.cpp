...
const char* getStatusString(TycheCash::HttpResponse::HTTP_STATUS status) {
  switch (status) {
  case TycheCash::HttpResponse::STATUS_200:
    return "200 OK";
  case TycheCash::HttpResponse::STATUS_404:
    return "404 Not Found";
  case TycheCash::HttpResponse::STATUS_500:
...
const char* getErrorBody(TycheCash::HttpResponse::HTTP_STATUS status) {
  switch (status) {
  case TycheCash::HttpResponse::STATUS_404:
...
HttpResponse::HttpResponse() {
  status = STATUS_200;
  headers["Server"] = "TycheCash-based HTTP server";
}

...
TransfersSyncronizer::TransfersSyncronizer(const TycheCash::Currency& currency, IBlockchainSynchronizer& sync, INode& node) :
...
void TransfersSyncronizer::save(std::ostream& os) {
  ...
  TycheCash::BinaryOutputStreamSerializer s(stream);
...
void TransfersSyncronizer::load(std::istream& is) {
  ...
  TycheCash::BinaryInputStreamSerializer s(inputStream);
  uint32_t version = 0;

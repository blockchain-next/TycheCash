...
std::vector<Crypto::Hash> getBlockHashes(const TycheCash::CompleteBlock* blocks, size_t count) {
...
TransfersConsumer::TransfersConsumer(const TycheCash::Currency& currency, INode& node, const SecretKey& viewSecret) :
...
std::error_code TransfersConsumer::onPoolUpdated(const std::vector<std::unique_ptr<ITransactionReader>>& addedTransactions, const std::vector<Hash>& deletedTransactions) {
  ...
  std::error_code processingError;
  for (auto& TycheCashTransaction : addedTransactions) {
    m_poolTxs.emplace(TycheCashTransaction->getTransactionHash());
    processingError = processTransaction(unconfirmedBlockInfo, *TycheCashTransaction.get());
    if (processingError) {
...
std::error_code createTransfers(
  ...
  for (auto idx : outputs) {
    ...
    if (outType == TransactionTypes::OutputType::Key) {
      ...
      TycheCash::generate_key_image_helper(
        account,
        txPubKey,
        idx,
        in_ephemeral,

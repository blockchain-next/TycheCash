...
void JsonRpcServer::processRequest(const TycheCash::HttpRequest& req, TycheCash::HttpResponse& resp) {
  try {
    ...
    if (req.getUrl() == "/json_rpc") {
      ...
      try {
        ...
        resp.setStatus(TycheCash::HttpResponse::STATUS_200);
      ...
      resp.setStatus(TycheCash::HttpResponse::STATUS_404);
      return;
    }
  } catch (std::exception& e) {
    logger(Logging::WARNING) << "Error while processing http request: " << e.what();
    resp.setStatus(TycheCash::HttpResponse::STATUS_500);
  }
}

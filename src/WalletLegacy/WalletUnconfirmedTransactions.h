...
template<> 
struct hash<TycheCash::TransactionOutputId> {
  size_t operator()(const TycheCash::TransactionOutputId &_v) const {    
...
public:
  ...
  bool serialize(TycheCash::ISerializer& s);

  bool findTransactionId(const Crypto::Hash& hash, TransactionId& id);
  void erase(const Crypto::Hash& hash);
  void add(const TycheCash::Transaction& tx, TransactionId transactionId, 

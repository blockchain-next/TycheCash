...
{
public:
  ContextCounterHolder(TycheCash::WalletAsyncContextCounter& shutdowner) : m_shutdowner(shutdowner) {}
  ~ContextCounterHolder() { m_shutdowner.delAsyncContext(); }

private:
  TycheCash::WalletAsyncContextCounter& m_shutdowner;
};
...
class InitWaiter : public TycheCash::IWalletLegacyObserver {
public:
...
class SaveWaiter : public TycheCash::IWalletLegacyObserver {
public:
...
class SyncStarter : public TycheCash::IWalletLegacyObserver {
public:
...
WalletLegacy::WalletLegacy(const TycheCash::Currency& currency, INode& node) :
...
void WalletLegacy::doLoad(std::istream& source) {
  ...
  try {
    ...
    runAtomic(m_cacheMutex, [this] () {this->m_state = WalletLegacy::NOT_INITIALIZED;} );
    m_observerManager.notify(&IWalletLegacyObserver::initCompleted, make_error_code(TycheCash::error::INTERNAL_WALLET_ERROR));
    return;
  }
...
void WalletLegacy::save(std::ostream& destination, bool saveDetailed, bool saveCache) {
  if(m_isStopping) {
    m_observerManager.notify(&IWalletLegacyObserver::saveCompleted, make_error_code(TycheCash::error::OPERATION_CANCELLED));
    return;
  }

  {
    ...
    throwIf(m_state != INITIALIZED, TycheCash::error::WRONG_STATE);
...
void WalletLegacy::doSave(std::ostream& destination, bool saveDetailed, bool saveCache) {
  ...
  try {
    ...
    runAtomic(m_cacheMutex, [this] () {this->m_state = WalletLegacy::INITIALIZED;} );
    m_observerManager.notify(&IWalletLegacyObserver::saveCompleted, make_error_code(TycheCash::error::INTERNAL_WALLET_ERROR));
    return;
  }
...
std::error_code WalletLegacy::changePassword(const std::string& oldPassword, const std::string& newPassword) {
  ...
  if (m_password.compare(oldPassword))
    return make_error_code(TycheCash::error::WRONG_PASSWORD);
...
std::error_code WalletLegacy::cancelTransaction(size_t transactionId) {
  return make_error_code(TycheCash::error::TX_CANCEL_IMPOSSIBLE);
}

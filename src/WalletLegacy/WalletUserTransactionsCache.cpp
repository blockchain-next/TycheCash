...
bool WalletUserTransactionsCache::serialize(TycheCash::ISerializer& s) {
...
void WalletUserTransactionsCache::updateTransaction(
  TransactionId transactionId, const TycheCash::Transaction& tx, uint64_t amount, const std::list<TransactionOutputInformation>& usedOutputs) {
...
std::shared_ptr<WalletLegacyEvent> WalletUserTransactionsCache::onTransactionUpdated(const TransactionInformation& txInfo, int64_t txBalance) {
  ...
  TransactionId id = TycheCash::WALLET_LEGACY_INVALID_TRANSACTION_ID;
  ...
  if (id == TycheCash::WALLET_LEGACY_INVALID_TRANSACTION_ID) {
    WalletLegacyTransaction transaction;
...
TransactionId WalletUserTransactionsCache::findTransactionByHash(const Hash& hash) {
  ...
  if (it == m_transactions.end())
    return TycheCash::WALLET_LEGACY_INVALID_TRANSACTION_ID;

...
const TycheCash::Currency PaymentGateService::getCurrency() {
...
void PaymentGateService::runInProcess(Logging::LoggerRef& log) {
  ...
  TycheCash::core core(currency, NULL, logger);

  TycheCash::TycheCashProtocolHandler protocol(currency, *dispatcher, core, NULL, logger);
  TycheCash::NodeServer p2pNode(*dispatcher, protocol, logger);

  protocol.set_p2p_endpoint(&p2pNode);
  core.set_TycheCash_protocol(&protocol);
  ...
  std::unique_ptr<TycheCash::INode> node(new TycheCash::InProcessNode(core, protocol));
...
void PaymentGateService::runRpcProxy(Logging::LoggerRef& log) {
  ...
  std::unique_ptr<TycheCash::INode> node(
...
void PaymentGateService::runWalletService(const TycheCash::Currency& currency, TycheCash::INode& node) {
  PaymentService::WalletConfiguration walletConfiguration{
    config.gateConfiguration.containerFile,
    config.gateConfiguration.containerPassword
  };

  std::unique_ptr<TycheCash::IWallet> wallet (WalletFactory::createWallet(currency, node, *dispatcher));

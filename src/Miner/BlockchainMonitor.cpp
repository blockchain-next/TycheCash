...
Crypto::Hash BlockchainMonitor::requestLastBlockHash() {
  ...
  try {
    TycheCash::HttpClient client(m_dispatcher, m_daemonHost, m_daemonPort);

    TycheCash::COMMAND_RPC_GET_LAST_BLOCK_HEADER::request request;
    TycheCash::COMMAND_RPC_GET_LAST_BLOCK_HEADER::response response;

    System::EventLock lk(m_httpEvent);
    TycheCash::JsonRpc::invokeJsonRpcCommand(client, "getlastblockheader", request, response);

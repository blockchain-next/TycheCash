...
struct TransferCommand {
  ...
  std::vector<TycheCash::WalletLegacyTransfer> dsts;
  std::vector<uint8_t> extra;
  uint64_t fee;

  TransferCommand(const TycheCash::Currency& currency) :
      ...
      while (!ar.eof()) {
        ...
        if (arg.size() && arg[0] == '-') {
          ...
          if (!m_currency.parseAccountAddressString(arg, de.addr)) {
            ...
            if (TycheCash::parsePaymentId(arg, paymentId)) {
...
std::string tryToOpenWalletOrLoadKeysOrThrow(LoggerRef& logger, std::unique_ptr<IWalletLegacy>& wallet, const std::string& walletFile, const std::string& password) {
  ...
  if (walletExists) {
    ...
    if (initError) { //bad password, or legacy format
      if (keysExists) {
        std::stringstream ss;
        TycheCash::importLegacyKeys(keys_file, password, ss);
        ...
        try {
          TycheCash::WalletHelper::storeWallet(*wallet, walletFileName);
...
simple_wallet::simple_wallet(System::Dispatcher& dispatcher, const TycheCash::Currency& currency, Logging::LoggerManager& log) :
...
bool simple_wallet::new_wallet(const std::string &wallet_file, const std::string& password) {
  ...
  try {
    ...
    try {
      TycheCash::WalletHelper::storeWallet(*m_wallet, m_wallet_file);
    } catch (std::exception& e) {
...
//----------------------------------------------------------------------------------------------------
void simple_wallet::externalTransactionCreated(TycheCash::TransactionId transactionId)  {
...
bool simple_wallet::show_payments(const std::vector<std::string> &args) {
  ...
  for (const std::string& arg: args) {
    ...
    if (TycheCash::parsePaymentId(arg, expectedPaymentId)) {
      size_t transactionsCount = m_wallet->getTransactionCount();
      for (size_t trantransactionNumber = 0; trantransactionNumber < transactionsCount; ++trantransactionNumber) {
        WalletLegacyTransaction txInfo;
        m_wallet->getTransaction(trantransactionNumber, txInfo);
        ...
        Crypto::Hash paymentId;
        if (TycheCash::getPaymentIdFromTxExtra(extraVec, paymentId) && paymentId == expectedPaymentId) {
          payments_found = true;
...
bool simple_wallet::transfer(const std::vector<std::string> &args) {
  try {
    ...
    if (!cmd.parseArguments(logger, args))
      return false;
    TycheCash::WalletHelper::SendCompleteResultObserver sent;
    ...
    TycheCash::TransactionId tx = m_wallet->sendTransaction(cmd.dsts, cmd.fee, extraString, cmd.fake_outs_count, 0);
...
int main(int argc, char* argv[]) {
  ...
  bool r = command_line::handle_error_helper(desc_all, [&]() {
    ...
    if (command_line::get_arg(vm, command_line::arg_help)) {
      TycheCash::Currency tmp_currency = TycheCash::CurrencyBuilder(logManager).currency();
      TycheCash::simple_wallet tmp_wallet(dispatcher, tmp_currency, logManager);

      std::cout << TycheCash_NAME << " wallet v" << PROJECT_VERSION_LONG << std::endl;
      ...
      std::cout << TycheCash_NAME << " wallet v" << PROJECT_VERSION_LONG;
      return false;
    }
  ...
  logger(INFO, BRIGHT_WHITE) << TycheCash_NAME << " wallet v" << PROJECT_VERSION_LONG;

  TycheCash::Currency currency = TycheCash::CurrencyBuilder(logManager).
  ...
  if (command_line::has_arg(vm, Tools::wallet_rpc_server::arg_rpc_bind_port)) {
    ...
    TycheCash::simple_wallet wal(dispatcher, currency, logManager);

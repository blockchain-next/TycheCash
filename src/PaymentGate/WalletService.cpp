...
Crypto::Hash parsePaymentId(const std::string& paymentIdStr) {
  if (!checkPaymentId(paymentIdStr)) {
    throw std::system_error(make_error_code(TycheCash::error::WalletServiceErrorCode::WRONG_PAYMENT_ID_FORMAT));
  }
...
bool getPaymentIdFromExtra(const std::string& binaryString, Crypto::Hash& paymentId) {
  return TycheCash::getPaymentIdFromTxExtra(Common::asBinaryArray(binaryString), paymentId);
}
...
struct TransactionsInBlockInfoFilter {
  ...
  bool checkTransaction(const TycheCash::WalletTransactionWithTransfers& transaction) const {
    if (havePaymentId) {
    ...
    for (const TycheCash::WalletTransfer& transfer: transaction.transfers) {
...
void addPaymentIdToExtra(const std::string& paymentId, std::string& extra) {
  ...
  if (!TycheCash::createTxExtraWithPaymentId(paymentId, extraVector)) {
...
Crypto::Hash parseHash(const std::string& hashString, Logging::LoggerRef logger) {
  ...
  if (!Common::podFromHex(hashString, hash)) {
    ...
    throw std::system_error(make_error_code(TycheCash::error::WalletServiceErrorCode::WRONG_HASH_FORMAT));
  }

  return hash;
}

std::vector<TycheCash::TransactionsInBlockInfo> filterTransactions(
  const std::vector<TycheCash::TransactionsInBlockInfo>& blocks,
  const TransactionsInBlockInfoFilter& filter) {

  std::vector<TycheCash::TransactionsInBlockInfo> result;

  for (const auto& block: blocks) {
    TycheCash::TransactionsInBlockInfo item;
    item.blockHash = block.blockHash;

    for (const auto& transaction: block.transactions) {
      if (transaction.transaction.state != TycheCash::WalletTransactionState::DELETED && filter.checkTransaction(transaction)) {
...
PaymentService::TransactionRpcInfo convertTransactionWithTransfersToTransactionRpcInfo(
  const TycheCash::WalletTransactionWithTransfers& transactionWithTransfers) {
  ...
  for (const TycheCash::WalletTransfer& transfer: transactionWithTransfers.transfers) {
...
std::vector<PaymentService::TransactionsInBlockRpcInfo> convertTransactionsInBlockInfoToTransactionsInBlockRpcInfo(
  ...
  for (const auto& block: blocks) {
    ...
    for (const TycheCash::WalletTransactionWithTransfers& transactionWithTransfers: block.transactions) {
...
std::vector<PaymentService::TransactionHashesInBlockRpcInfo> convertTransactionsInBlockInfoToTransactionHashesInBlockRpcInfo(
  ...
  for (const TycheCash::TransactionsInBlockInfo& block: blocks) {
    PaymentService::TransactionHashesInBlockRpcInfo item;
    item.blockHash = Common::podToHex(block.blockHash);

    for (const TycheCash::WalletTransactionWithTransfers& transaction: block.transactions) {
...
void validateAddresses(const std::vector<std::string>& addresses, const TycheCash::Currency& currency, Logging::LoggerRef logger) {
...
std::vector<TycheCash::WalletOrder> convertWalletRpcOrdersToWalletOrders(const std::vector<PaymentService::WalletRpcOrder>& orders) {
  std::vector<TycheCash::WalletOrder> result;
  result.reserve(orders.size());

  for (const auto& order: orders) {
    result.emplace_back(TycheCash::WalletOrder {order.address, order.amount});
  }
...
void saveWallet(TycheCash::IWallet& wallet, std::fstream& walletFile, bool saveDetailed = true, bool saveCache = true) {
  wallet.save(walletFile, saveDetailed, saveCache);
  walletFile.flush();
}

void secureSaveWallet(TycheCash::IWallet& wallet, const std::string& path, bool saveDetailed = true, bool saveCache = true) {
...
void generateNewWallet(const TycheCash::Currency &currency, const WalletConfiguration &conf, Logging::ILogger& logger, System::Dispatcher& dispatcher) {
  Logging::LoggerRef log(logger, "generateNewWallet");

  TycheCash::INode* nodeStub = NodeFactory::createNodeStub();
  std::unique_ptr<TycheCash::INode> nodeGuard(nodeStub);

  TycheCash::IWallet* wallet = WalletFactory::createWallet(currency, *nodeStub, dispatcher);
  std::unique_ptr<TycheCash::IWallet> walletGuard(wallet);
...
void importLegacyKeys(const std::string &legacyKeysFile, const WalletConfiguration &conf) {
  ...
  TycheCash::importLegacyKeys(legacyKeysFile, conf.walletPassword, archive);
...
WalletService::WalletService(const TycheCash::Currency& currency, System::Dispatcher& sys, TycheCash::INode& node,
  TycheCash::IWallet& wallet, const WalletConfiguration& conf, Logging::ILogger& logger) :
    currency(currency),
    wallet(wallet),
    node(node),
    config(conf),
...
std::error_code WalletService::resetWallet() {
  try {
    ...
    if (!inited) {
      ...
      return make_error_code(TycheCash::error::NOT_INITIALIZED);
    }
    ...
    logger(Logging::WARNING) << "Error while reseting wallet: " << x.what();
    return make_error_code(TycheCash::error::INTERNAL_WALLET_ERROR);
  }
...
std::error_code WalletService::replaceWithNewWallet(const std::string& viewSecretKeyText) {
  try {
    ...
    if (!Common::podFromHex(viewSecretKeyText, viewSecretKey)) {
      ...
      return make_error_code(TycheCash::error::WalletServiceErrorCode::WRONG_KEY_FORMAT);
    }
...
std::error_code WalletService::getSpendkeys(const std::string& address, std::string& publicSpendKeyText, std::string& secretSpendKeyText) {
  try {
    ...
    TycheCash::KeyPair key = wallet.getAddressSpendKey(address);
...
std::error_code WalletService::getViewKey(std::string& viewSecretKey) {
  try {
    System::EventLock lk(readyEvent);
    TycheCash::KeyPair viewKey = wallet.getViewKey();
...
std::error_code WalletService::getTransaction(const std::string& transactionHash, TransactionRpcInfo& transaction) {
  try {
    ...
    TycheCash::WalletTransactionWithTransfers transactionWithTransfers = wallet.getTransaction(hash);

    if (transactionWithTransfers.transaction.state == TycheCash::WalletTransactionState::DELETED) {
      logger(Logging::WARNING) << "Transaction " << transactionHash << " is deleted";
      return make_error_code(TycheCash::error::OBJECT_NOT_FOUND);
    }
...
std::error_code WalletService::sendTransaction(const SendTransaction::Request& request, std::string& transactionHash) {
  try {
    ...
    TycheCash::TransactionParameters sendParams;
...
std::error_code WalletService::deleteDelayedTransaction(const std::string& transactionHash) {
  try {
    ...
    if (idIt == transactionIdIndex.end()) {
      return make_error_code(TycheCash::error::WalletServiceErrorCode::OBJECT_NOT_FOUND);
    }
...
std::error_code WalletService::getUnconfirmedTransactionHashes(const std::vector<std::string>& addresses, std::vector<std::string>& transactionHashes) {
  try {
    ...
    std::vector<TycheCash::WalletTransactionWithTransfers> transactions = wallet.getUnconfirmedTransactions();
...
void WalletService::refresh() {
  try {
    ...
    for (;;) {
      ...
      if (event.type == TycheCash::TRANSACTION_CREATED) {
...
std::vector<TycheCash::TransactionsInBlockInfo> WalletService::getTransactions(const Crypto::Hash& blockHash, size_t blockCount) const {
  std::vector<TycheCash::TransactionsInBlockInfo> result = wallet.getTransactions(blockHash, blockCount);
  if (result.empty()) {
    throw std::system_error(make_error_code(TycheCash::error::WalletServiceErrorCode::OBJECT_NOT_FOUND));
  }

  return result;
}

std::vector<TycheCash::TransactionsInBlockInfo> WalletService::getTransactions(uint32_t firstBlockIndex, size_t blockCount) const {
  std::vector<TycheCash::TransactionsInBlockInfo> result = wallet.getTransactions(firstBlockIndex, blockCount);
  if (result.empty()) {
    throw std::system_error(make_error_code(TycheCash::error::WalletServiceErrorCode::OBJECT_NOT_FOUND));
  }
...
std::vector<TransactionHashesInBlockRpcInfo> WalletService::getRpcTransactionHashes(const Crypto::Hash& blockHash, size_t blockCount, const TransactionsInBlockInfoFilter& filter) const {
  std::vector<TycheCash::TransactionsInBlockInfo> allTransactions = getTransactions(blockHash, blockCount);
  std::vector<TycheCash::TransactionsInBlockInfo> filteredTransactions = filterTransactions(allTransactions, filter);
...
std::vector<TransactionHashesInBlockRpcInfo> WalletService::getRpcTransactionHashes(uint32_t firstBlockIndex, size_t blockCount, const TransactionsInBlockInfoFilter& filter) const {
  std::vector<TycheCash::TransactionsInBlockInfo> allTransactions = getTransactions(firstBlockIndex, blockCount);
  std::vector<TycheCash::TransactionsInBlockInfo> filteredTransactions = filterTransactions(allTransactions, filter);
  return convertTransactionsInBlockInfoToTransactionHashesInBlockRpcInfo(filteredTransactions);
}

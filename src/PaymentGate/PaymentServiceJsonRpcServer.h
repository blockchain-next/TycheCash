...
class PaymentServiceJsonRpcServer : public TycheCash::JsonRpcServer {
public:
...
private:
  ...
  HandlerFunction jsonHandler(RequestHandler handler) {
    return [handler] (const Common::JsonValue& jsonRpcParams, Common::JsonValue& jsonResponse) mutable {
      ...
      try {
        TycheCash::JsonInputValueSerializer inputSerializer(const_cast<Common::JsonValue&>(jsonRpcParams));
      ...
      TycheCash::JsonOutputStreamSerializer outputSerializer;

...
bool GetRandomOutputs::request(TycheCash::core& c, uint64_t amount, size_t mixin, TycheCash::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& resp) {
  TycheCash::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_request req;

  req.amounts.push_back(amount);
  req.outs_count = mixin;

  resp = boost::value_initialized<TycheCash::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response>();
...
bool GetRandomOutputs::checkHalfUnlocked(TycheCash::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  TycheCash::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;
...
bool GetRandomOutputs::checkFullyUnlocked(TycheCash::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  TycheCash::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;

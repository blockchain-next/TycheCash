...
bool one_block::verify_1(TycheCash::core& c, size_t ev_index, const std::vector<test_event_entry> &events)
{
    DEFINE_TESTS_ERROR_CONTEXT("one_block::verify_1");

    alice = boost::get<TycheCash::AccountBase>(events[1]);

    // check balances
    //std::vector<const TycheCash::Block*> chain;
    //map_hash2tx_t mtx;
    //CHECK_TEST_CONDITION(find_block_chain(events, chain, mtx, get_block_hash(boost::get<TycheCash::Block>(events[1]))));
    ...
    CHECK_TEST_CONDITION(blocks.back() == boost::get<TycheCash::Block>(events[0]));
...
bool gen_simple_chain_001::generate(std::vector<test_event_entry> &events)
{
    ...
    map_hash2tx_t mtx;
    /*bool r = */find_block_chain(events, chain, mtx, get_block_hash(boost::get<TycheCash::Block>(events[3])));
...
bool gen_simple_chain_001::verify_callback_1(TycheCash::core& c, size_t ev_index, const std::vector<test_event_entry> &events)
{
  return true;
}

bool gen_simple_chain_001::verify_callback_2(TycheCash::core& c, size_t ev_index, const std::vector<test_event_entry> &events)
{
  return true;
}

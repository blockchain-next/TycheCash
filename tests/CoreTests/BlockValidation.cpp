...
namespace {
  bool lift_up_difficulty(const TycheCash::Currency& currency, std::vector<test_event_entry>& events,
                          std::vector<uint64_t>& timestamps,
                          std::vector<TycheCash::difficulty_type>& cummulative_difficulties, test_generator& generator,
                          size_t new_block_count, const TycheCash::Block blk_last,
                          const TycheCash::AccountBase& miner_account, uint8_t block_major_version) {
    TycheCash::difficulty_type commulative_diffic = cummulative_difficulties.empty() ? 0 : cummulative_difficulties.back();
    TycheCash::Block blk_prev = blk_last;
    for (size_t i = 0; i < new_block_count; ++i) {
      TycheCash::Block blk_next;
      TycheCash::difficulty_type diffic = currency.nextDifficulty(timestamps, cummulative_difficulties);
  ...
  void clearTransaction(TycheCash::Transaction& tx) {
    tx.version = 0;
    tx.unlockTime = 0;
    tx.inputs.clear();
    tx.outputs.clear();
...
bool gen_block_invalid_prev_id::check_block_verification_context(const TycheCash::block_verification_context& bvc, size_t event_idx, const TycheCash::Block& /*blk*/)
{
  if (1 == event_idx)
...
gen_block_invalid_binary_format::gen_block_invalid_binary_format() : 
  ...
  TycheCash::CurrencyBuilder currencyBuilder(m_logger);
...
bool gen_block_invalid_binary_format::check_block_verification_context(const TycheCash::block_verification_context& bvc,
                                                                       size_t event_idx, const TycheCash::Block& blk)
{
...
bool gen_block_invalid_binary_format::corrupt_blocks_boundary(TycheCash::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  m_corrupt_blocks_begin_idx = ev_index + 1;
  return true;
}

bool gen_block_invalid_binary_format::check_all_blocks_purged(TycheCash::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{

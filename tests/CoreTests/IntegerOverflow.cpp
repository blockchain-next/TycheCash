...
namespace
{
  ...
  void append_TransactionSourceEntry(std::vector<TycheCash::TransactionSourceEntry>& sources, const Transaction& tx, size_t out_idx)
  {
    TycheCash::TransactionSourceEntry se;
    se.amount = tx.outputs[out_idx].amount;
    se.outputs.push_back(std::make_pair(0, boost::get<TycheCash::KeyOutput>(tx.outputs[out_idx].target).key));
    se.realOutput = 0;
...
bool gen_uint_overflow_base::check_tx_verification_context(const TycheCash::tx_verification_context& tvc, bool tx_added, size_t event_idx, const TycheCash::Transaction& /*tx*/)
{
  return m_last_valid_block_event_idx < event_idx ? !tx_added && tvc.m_verification_failed : tx_added && !tvc.m_verification_failed;
}

bool gen_uint_overflow_base::check_block_verification_context(const TycheCash::block_verification_context& bvc, size_t event_idx, const TycheCash::Block& /*block*/)
{
...
bool gen_uint_overflow_base::mark_last_valid_block(TycheCash::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
...
bool gen_uint_overflow_1::generate(std::vector<test_event_entry>& events) const
{
  ...
  std::list<TycheCash::Transaction> txs_1;

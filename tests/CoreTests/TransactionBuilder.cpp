...
TransactionBuilder::TransactionBuilder(const TycheCash::Currency& currency, uint64_t unlockTime)
  : m_currency(currency), m_version(TycheCash::CURRENT_TRANSACTION_VERSION), m_unlockTime(unlockTime), m_txKey(generateKeyPair()) {}
...
TransactionBuilder& TransactionBuilder::setTxKeys(const TycheCash::KeyPair& txKeys) {
  m_txKey = txKeys;
  return *this;
}

TransactionBuilder& TransactionBuilder::setInput(const std::vector<TycheCash::TransactionSourceEntry>& sources, const TycheCash::AccountKeys& senderKeys) {
  m_sources = sources;
...
TransactionBuilder& TransactionBuilder::setOutput(const std::vector<TycheCash::TransactionDestinationEntry>& destinations) {
  m_destinations = destinations;
  return *this;
}

TransactionBuilder& TransactionBuilder::addOutput(const TycheCash::TransactionDestinationEntry& dest) {
...
Transaction TransactionBuilder::build() const {
  ...
  std::vector<TycheCash::KeyPair> contexts;
...
void TransactionBuilder::fillInputs(Transaction& tx, std::vector<TycheCash::KeyPair>& contexts) const {
...
void TransactionBuilder::signSources(const Crypto::Hash& prefixHash, const std::vector<TycheCash::KeyPair>& contexts, Transaction& tx) const {

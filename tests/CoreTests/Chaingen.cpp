...
//#include "TycheCash_boost_serialization.h"

using namespace std;
using namespace TycheCash;

struct output_index {
    const TycheCash::TransactionOutputTarget out;
    uint64_t amount;
    ...
    uint32_t idx;
    bool spent;
    const TycheCash::Block *p_blk;
    const TycheCash::Transaction *p_tx;

    output_index(const TycheCash::TransactionOutputTarget &_out, uint64_t _a, size_t _h, size_t tno, size_t ono, const TycheCash::Block *_pb, const TycheCash::Transaction *_pt)
...
bool init_output_indices(map_output_idx_t& outs, std::map<uint64_t, std::vector<size_t> >& outs_mine, const std::vector<TycheCash::Block>& blockchain, const map_hash2tx_t& mtx, const TycheCash::AccountBase& from) {
...
bool init_spent_output_indices(map_output_idx_t& outs, map_output_t& outs_mine, const std::vector<TycheCash::Block>& blockchain, const map_hash2tx_t& mtx, const TycheCash::AccountBase& from) {
...
bool fill_tx_sources(std::vector<TransactionSourceEntry>& sources, const std::vector<test_event_entry>& events,
                     const Block& blk_head, const TycheCash::AccountBase& from, uint64_t amount, size_t nmix)
{
    map_output_idx_t outs;
    map_output_t outs_mine;

    std::vector<TycheCash::Block> blockchain;
    map_hash2tx_t mtx;
    ...
    BOOST_REVERSE_FOREACH(const map_output_t::value_type o, outs_mine)
    {
        for (size_t i = 0; i < o.second.size() && !sources_found; ++i)
        {
            ...
            TycheCash::TransactionSourceEntry ts;
            ts.amount = oi.amount;
...
bool fill_tx_destination(TransactionDestinationEntry &de, const TycheCash::AccountBase &to, uint64_t amount) {
...
bool construct_tx_to_key(Logging::ILogger& logger, const std::vector<test_event_entry>& events, TycheCash::Transaction& tx, const Block& blk_head,
                         const TycheCash::AccountBase& from, const TycheCash::AccountBase& to, uint64_t amount,
...
const TycheCash::Currency& test_chain_unit_base::currency() const
{
  return m_currency;
}
...
bool test_chain_unit_base::verify(const std::string& cb_name, TycheCash::core& c, size_t ev_index, const std::vector<test_event_entry> &events)
{

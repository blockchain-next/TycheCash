...
struct serialized_object
{
  ...
  serialized_object(const TycheCash::BinaryArray& a_data)
    : data(a_data)
  {
  }

  TycheCash::BinaryArray data;
  //BEGIN_SERIALIZE_OBJECT()
  //  FIELD(data)
  //  END_SERIALIZE()
...
typedef serialized_object<TycheCash::Block> serialized_block;
typedef serialized_object<TycheCash::Transaction> serialized_transaction;
...
//VARIANT_TAG(binary_archive, TycheCash::account_base, 0xcc);
//VARIANT_TAG(binary_archive, serialized_block, 0xcd);
//VARIANT_TAG(binary_archive, serialized_transaction, 0xce);
//VARIANT_TAG(binary_archive, event_visitor_settings, 0xcf);

typedef boost::variant<TycheCash::Block, TycheCash::Transaction, TycheCash::AccountBase, callback_entry, serialized_block, serialized_transaction, event_visitor_settings> test_event_entry;
typedef std::unordered_map<Crypto::Hash, const TycheCash::Transaction*> map_hash2tx_t;

class test_chain_unit_base: boost::noncopyable
{
public:
  test_chain_unit_base() :
    m_currency(TycheCash::CurrencyBuilder(m_logger).currency()) {
  }

  typedef std::function<bool (TycheCash::core& c, size_t ev_index, const std::vector<test_event_entry> &events)> verify_callback;
  typedef std::map<std::string, verify_callback> callbacks_map;

  const TycheCash::Currency& currency() const;
  void register_callback(const std::string& cb_name, verify_callback cb);
  bool verify(const std::string& cb_name, TycheCash::core& c, size_t ev_index, const std::vector<test_event_entry> &events);
...
bool construct_tx_to_key(Logging::ILogger& logger, const std::vector<test_event_entry>& events, TycheCash::Transaction& tx,
                         const TycheCash::Block& blk_head, const TycheCash::AccountBase& from, const TycheCash::AccountBase& to,
                         uint64_t amount, uint64_t fee, size_t nmix);
TycheCash::Transaction construct_tx_with_fee(Logging::ILogger& logger, std::vector<test_event_entry>& events, const TycheCash::Block& blk_head,
                                            const TycheCash::AccountBase& acc_from, const TycheCash::AccountBase& acc_to,
                                            uint64_t amount, uint64_t fee);

void get_confirmed_txs(const std::vector<TycheCash::Block>& blockchain, const map_hash2tx_t& mtx, map_hash2tx_t& confirmed_txs);
bool find_block_chain(const std::vector<test_event_entry>& events, std::vector<TycheCash::Block>& blockchain, map_hash2tx_t& mtx, const Crypto::Hash& head);
void fill_tx_sources_and_destinations(const std::vector<test_event_entry>& events, const TycheCash::Block& blk_head,
                                      const TycheCash::AccountBase& from, const TycheCash::AccountBase& to,
                                      uint64_t amount, uint64_t fee, size_t nmix,
                                      std::vector<TycheCash::TransactionSourceEntry>& sources,
                                      std::vector<TycheCash::TransactionDestinationEntry>& destinations);
uint64_t get_balance(const TycheCash::AccountBase& addr, const std::vector<TycheCash::Block>& blockchain, const map_hash2tx_t& mtx);

//--------------------------------------------------------------------------
template<class t_test_class>
auto do_check_tx_verification_context(const TycheCash::tx_verification_context& tvc, bool tx_added, size_t event_index, const TycheCash::Transaction& tx, t_test_class& validator, int)
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool check_tx_verification_context(const TycheCash::tx_verification_context& tvc, bool tx_added, size_t event_index, const TycheCash::Transaction& tx, t_test_class& validator)
{
  // SFINAE in action
...
//--------------------------------------------------------------------------
template<class t_test_class>
auto do_check_block_verification_context(const TycheCash::block_verification_context& bvc, size_t event_index, const TycheCash::Block& blk, t_test_class& validator, int)
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool check_block_verification_context(const TycheCash::block_verification_context& bvc, size_t event_index, const TycheCash::Block& blk, t_test_class& validator)
{
  // SFINAE in action
...
public:
  push_core_event_visitor(TycheCash::core& c, const std::vector<test_event_entry>& events, t_test_class& validator)
    : m_c(c)
    , m_events(events)
  ...
  bool operator()(const TycheCash::Transaction& tx) const
  {
    log_event("TycheCash::Transaction");
  ...
  bool operator()(const TycheCash::Block& b) const
  {
    log_event("TycheCash::Block");

    TycheCash::block_verification_context bvc = boost::value_initialized<decltype(bvc)>();
  ...
  bool operator()(const TycheCash::AccountBase& ab) const
  {
    log_event("TycheCash::account_base");
    return true;
  }
  ...
  bool operator()(const serialized_block& sr_block) const
  {
    ...
    TycheCash::Block blk;
    if (!TycheCash::fromBinaryArray(blk, sr_block.data)) {
      blk = TycheCash::Block();
    }
  ...
  bool operator()(const serialized_transaction& sr_tx) const
  {
    ...
    if (!TycheCash::fromBinaryArray(tx, sr_tx.data)) {
      tx = TycheCash::Transaction();
    }
...
//--------------------------------------------------------------------------
template<class t_test_class>
inline bool replay_events_through_core(TycheCash::core& cr, const std::vector<test_event_entry>& events, t_test_class& validator)
{
  try {
    CHECK_AND_ASSERT_MES(typeid(TycheCash::Block) == events[0].type(), false, "First event must be genesis block creation");
    cr.set_genesis_block(boost::get<TycheCash::Block>(events[0]));
...
inline bool do_replay_events(std::vector<test_event_entry>& events, t_test_class& validator)
{
  ...
  TycheCash::CoreConfig::initOptions(desc);
  ...
  coreConfig.init(vm);
  TycheCash::MinerConfig emptyMinerConfig;
  TycheCash::TycheCash_protocol_stub pr; //TODO: stub only for this kind of test, make real validation of relayed objects
  TycheCash::core c(validator.currency(), &pr, logger);
...
#define MAKE_NEXT_BLOCK_TX1(VEC_EVENTS, BLK_NAME, PREV_BLOCK, MINER_ACC, TX1)         \
  TycheCash::Block BLK_NAME;                                                         \
  {                                                                                   \
    std::list<TycheCash::Transaction> tx_list;                                       \
...
#define REWIND_BLOCKS_N(VEC_EVENTS, BLK_NAME, PREV_BLOCK, MINER_ACC, COUNT)           \
  TycheCash::Block BLK_NAME;                                                         \
  {                                                                                   \
    TycheCash::Block blk_last = PREV_BLOCK;                                          \
...
#define MAKE_TX_LIST_START(VEC_EVENTS, SET_NAME, FROM, TO, AMOUNT, HEAD) \
    std::list<TycheCash::Transaction> SET_NAME; \
...
const uint64_t COIN = Pow10<TycheCash::parameters::TycheCash_DISPLAY_DECIMAL_POINT>::value;

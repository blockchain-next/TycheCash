...
{
  TycheCash::AccountPublicAddress addr;

  ASSERT_NO_THROW(TycheCash::loadFromBinary(addr, Common::asBinaryArray(test_serialized_keys)));
  std::string addr_str = TycheCash::getAccountAddressAsStr(TEST_PUBLIC_ADDRESS_BASE58_PREFIX, addr);
...
TEST(parseAccountAddressString, handles_valid_address)
{
  uint64_t prefix;
  TycheCash::AccountPublicAddress addr;
  ASSERT_TRUE(TycheCash::parseAccountAddressString(prefix, addr, test_keys_addr_str));
  ASSERT_EQ(TEST_PUBLIC_ADDRESS_BASE58_PREFIX, prefix);

  BinaryArray blob;
  ASSERT_NO_THROW(blob = TycheCash::storeToBinary(addr));
...
TEST(parseAccountAddressString, fails_on_invalid_address_format)
{
  ...
  uint64_t prefix;
  TycheCash::AccountPublicAddress addr;
  ASSERT_FALSE(TycheCash::parseAccountAddressString(prefix, addr, addr_str));
}

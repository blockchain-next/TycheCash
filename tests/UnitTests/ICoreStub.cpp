...
#include "TycheCashCore/IBlock.h"
...
ICoreStub::ICoreStub(const TycheCash::Block& genesisBlock) :
    topHeight(0),
    globalIndicesResult(false),
    randomOutsResult(false),
    poolTxVerificationResult(true),
...
bool ICoreStub::addObserver(TycheCash::ICoreObserver* observer) {
  return true;
}

bool ICoreStub::removeObserver(TycheCash::ICoreObserver* observer) {
  return true;
}
...
bool ICoreStub::get_random_outs_for_amounts(const TycheCash::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_request& req,
    TycheCash::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& res) {
  res = randomOuts;
...
TycheCash::i_TycheCash_protocol* ICoreStub::get_protocol() {
  return nullptr;
}

bool ICoreStub::handle_incoming_tx(TycheCash::BinaryArray const& tx_blob, TycheCash::tx_verification_context& tvc, bool keeped_by_block) {
  return true;
}
...
void ICoreStub::set_random_outs(const TycheCash::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& resp, bool result) {
  randomOuts = resp;
  randomOutsResult = result;
}

std::vector<TycheCash::Transaction> ICoreStub::getPoolTransactions() {
  return std::vector<TycheCash::Transaction>();
}

bool ICoreStub::getPoolChanges(const Crypto::Hash& tailBlockId, const std::vector<Crypto::Hash>& knownTxsIds,
                               std::vector<TycheCash::Transaction>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
  ...
  for (const std::pair<Crypto::Hash, TycheCash::Transaction>& poolEntry : transactionPool) {
...
bool ICoreStub::getPoolChangesLite(const Crypto::Hash& tailBlockId, const std::vector<Crypto::Hash>& knownTxsIds,
        std::vector<TycheCash::TransactionPrefixInfo>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
  std::vector<TycheCash::Transaction> added;
  bool returnStatus = getPoolChanges(tailBlockId, knownTxsIds, added, deletedTxsIds);

  for (const auto& tx : added) {
    TycheCash::TransactionPrefixInfo tpi;
    tpi.txPrefix = tx;
...
void ICoreStub::getPoolChanges(const std::vector<Crypto::Hash>& knownTxsIds, std::vector<TycheCash::Transaction>& addedTxs,
                               std::vector<Crypto::Hash>& deletedTxsIds) {
}

bool ICoreStub::queryBlocks(const std::vector<Crypto::Hash>& block_ids, uint64_t timestamp,
  uint32_t& start_height, uint32_t& current_height, uint32_t& full_offset, std::vector<TycheCash::BlockFullInfo>& entries) {
  //stub
  return true;
}

bool ICoreStub::queryBlocksLite(const std::vector<Crypto::Hash>& block_ids, uint64_t timestamp,
  uint32_t& start_height, uint32_t& current_height, uint32_t& full_offset, std::vector<TycheCash::BlockShortInfo>& entries) {
  //stub
  return true;
}
...
size_t ICoreStub::addChain(const std::vector<const TycheCash::IBlock*>& chain) {
  size_t blocksCounter = 0;
  for (const TycheCash::IBlock* block : chain) {
    for (size_t txNumber = 0; txNumber < block->getTransactionCount(); ++txNumber) {
      const TycheCash::Transaction& tx = block->getTransaction(txNumber);
      Crypto::Hash txHash = TycheCash::NULL_HASH;
      size_t blobSize = 0;
...
Crypto::Hash ICoreStub::getBlockIdByHeight(uint32_t height) {
  ...
  if (iter == blockHashByHeightIndex.end()) {
    return TycheCash::NULL_HASH;
  }
  return iter->second;
}

bool ICoreStub::getBlockByHash(const Crypto::Hash &h, TycheCash::Block &blk) {
...
void ICoreStub::getTransactions(const std::vector<Crypto::Hash>& txs_ids, std::list<TycheCash::Transaction>& txs, std::list<Crypto::Hash>& missed_txs, bool checkTxPool) {
...
bool ICoreStub::scanOutputkeysForIndices(const TycheCash::KeyInput& txInToKey, std::list<std::pair<Crypto::Hash, size_t>>& outputReferences) {
  return true;
}

bool ICoreStub::getBlockDifficulty(uint32_t height, TycheCash::difficulty_type& difficulty) {
  return true;
}
...
bool ICoreStub::getBlockContainingTx(const Crypto::Hash& txId, Crypto::Hash& blockId, uint32_t& blockHeight) {
  ...
  blockHeight = boost::get<TycheCash::BaseInput>(blockIter->second.baseTransaction.inputs.front()).blockIndex;
  return true;
}

bool ICoreStub::getMultisigOutputReference(const TycheCash::MultisignatureInput& txInMultisig, std::pair<Crypto::Hash, size_t>& outputReference) {
  return true;
}

void ICoreStub::addBlock(const TycheCash::Block& block) {
  uint32_t height = boost::get<TycheCash::BaseInput>(block.baseTransaction.inputs.front()).blockIndex;
  Crypto::Hash hash = TycheCash::get_block_hash(block);
  if (height > topHeight) {
    topHeight = height;
    topId = hash;
  }
  ...
  blockHashByTxHashIndex.emplace(std::make_pair(TycheCash::getObjectHash(block.baseTransaction), hash));
  for (auto txHash : block.transactionHashes) {
    blockHashByTxHashIndex.emplace(std::make_pair(txHash, hash));
  }
}

void ICoreStub::addTransaction(const TycheCash::Transaction& tx) {
  Crypto::Hash hash = TycheCash::getObjectHash(tx);
...
bool ICoreStub::getOrphanBlocksByHeight(uint32_t height, std::vector<TycheCash::Block>& blocks) {
  return true;
}

bool ICoreStub::getBlocksByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t blocksNumberLimit, std::vector<TycheCash::Block>& blocks, uint32_t& blocksNumberWithinTimestamps) {
  return true;
}

bool ICoreStub::getPoolTransactionsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t transactionsNumberLimit, std::vector<TycheCash::Transaction>& transactions, uint64_t& transactionsNumberWithinTimestamps) {
  return true;
}

bool ICoreStub::getTransactionsByPaymentId(const Crypto::Hash& paymentId, std::vector<TycheCash::Transaction>& transactions) {
  return true;
}
...
std::unique_ptr<TycheCash::IBlock> ICoreStub::getBlock(const Crypto::Hash& blockId) {
  return std::unique_ptr<TycheCash::IBlock>(nullptr);
}
...
bool ICoreStub::addMessageQueue(TycheCash::MessageQueue<TycheCash::BlockchainMessage>& messageQueuePtr) {
  return true;
}

bool ICoreStub::removeMessageQueue(TycheCash::MessageQueue<TycheCash::BlockchainMessage>& messageQueuePtr) {
  return true;
}

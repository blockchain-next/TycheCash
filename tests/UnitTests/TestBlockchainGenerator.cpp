...
public:
  ...
  void generate(const AccountPublicAddress& address, Transaction& tx, uint64_t unlockTime = 0)
  {
    ...
    TycheCash::decompose_amount_into_digits(this->m_source_amount, 0,
      [&](uint64_t chunk) { destinations.push_back(TycheCash::TransactionDestinationEntry(chunk, address)); },
      [&](uint64_t a_dust) { destinations.push_back(TycheCash::TransactionDestinationEntry(a_dust, address)); });

    TycheCash::constructTransaction(this->m_miners[this->real_source_idx].getAccountKeys(), this->m_sources, destinations, std::vector<uint8_t>(), tx, unlockTime, m_logger);
  }
...
TestBlockchainGenerator::TestBlockchainGenerator(const TycheCash::Currency& currency) :
  m_currency(currency),
  generator(currency)
{
...
std::vector<TycheCash::Block>& TestBlockchainGenerator::getBlockchain()
{
  std::unique_lock<std::mutex> lock(m_mutex);
  return m_blockchain;
}

std::vector<TycheCash::Block> TestBlockchainGenerator::getBlockchainCopy() {
  std::unique_lock<std::mutex> lock(m_mutex);

  std::vector<TycheCash::Block> blockchain(m_blockchain);
  return blockchain;
}

bool TestBlockchainGenerator::getTransactionByHash(const Crypto::Hash& hash, TycheCash::Transaction& tx, bool checkTxPool)
{
  std::unique_lock<std::mutex> lock(m_mutex);
...
const TycheCash::AccountBase& TestBlockchainGenerator::getMinerAccount() const {
  std::unique_lock<std::mutex> lock(m_mutex);
  return miner_acc;
}
...
void TestBlockchainGenerator::addGenesisBlock() {
  ...
  m_timestampIndex.add(m_currency.genesisBlock().timestamp, TycheCash::get_block_hash(m_currency.genesisBlock()));
...
void TestBlockchainGenerator::addMiningBlock() {
  ...
  TycheCash::Block& prev_block = m_blockchain.back();
  ...
  m_timestampIndex.add(block.timestamp, TycheCash::get_block_hash(block));
...
void TestBlockchainGenerator::addTxToBlockchain(const TycheCash::Transaction& transaction)
{
  std::unique_lock<std::mutex> lock(m_mutex);
  addToBlockchain(transaction);
}

bool TestBlockchainGenerator::getBlockRewardForAddress(const TycheCash::AccountPublicAddress& address)
{
  std::unique_lock<std::mutex> lock(m_mutex);
...
bool TestBlockchainGenerator::generateTransactionsInOneBlock(const TycheCash::AccountPublicAddress& address, size_t n) {
  std::unique_lock<std::mutex> lock(m_mutex);
...
bool TestBlockchainGenerator::getSingleOutputTransaction(const TycheCash::AccountPublicAddress& address, uint64_t amount) {
  std::unique_lock<std::mutex> lock(m_mutex);
...
void TestBlockchainGenerator::addToBlockchain(const TycheCash::Transaction& tx) {
  addToBlockchain(std::vector<TycheCash::Transaction> {tx});
}

void TestBlockchainGenerator::addToBlockchain(const std::vector<TycheCash::Transaction>& txs) {
  addToBlockchain(txs, miner_acc);
}

void TestBlockchainGenerator::addToBlockchain(const std::vector<TycheCash::Transaction>& txs, const TycheCash::AccountBase& minerAddress) {
  std::list<TycheCash::Transaction> txsToBlock;
  ...
  std::vector<TycheCash::Transaction>& new_txs, std::vector<Crypto::Hash>& deleted_tx_ids)
{
  ...
  if (known_block_id != TycheCash::get_block_hash(m_blockchain.back())) {
    is_bc_actual = false;
    return;
  }
...
void TestBlockchainGenerator::putTxToPool(const TycheCash::Transaction& tx) {
  std::unique_lock<std::mutex> lock(m_mutex);

  Crypto::Hash txHash = TycheCash::getObjectHash(tx);
  m_txPool[txHash] = tx;
}

void TestBlockchainGenerator::putTxPoolToBlockchain() {
  std::unique_lock<std::mutex> lock(m_mutex);
  std::vector<TycheCash::Transaction> txs;
...
void TestBlockchainGenerator::setMinerAccount(const TycheCash::AccountBase& account) {
  miner_acc = account;
}
...
bool TestBlockchainGenerator::getPoolTransactionIdsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t transactionsNumberLimit, std::vector<Crypto::Hash>& hashes, uint64_t& transactionsNumberWithinTimestamps) {
  ...
  for (auto i : m_txPool) {
    ...
    hashes.push_back(TycheCash::getObjectHash(i.second));
    ++c;
  }
  return true;
}
...
void TestBlockchainGenerator::addTx(const TycheCash::Transaction& tx) {
...
bool TestBlockchainGenerator::generateFromBaseTx(const TycheCash::AccountBase& address) {
  std::unique_lock<std::mutex> lock(m_mutex);

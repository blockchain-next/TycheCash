...
namespace {
TycheCash::Transaction createTx(TycheCash::ITransactionReader& tx) {
  TycheCash::Transaction outTx;
...
protected:
  ...
  ICoreStub coreStub;
  ITycheCashProtocolQueryStub protocolQueryStub;
  TycheCash::InProcessNode node;
...
TEST_F(InProcessNodeTests, initOk) {
  TycheCash::InProcessNode newNode(coreStub, protocolQueryStub);
  CallbackStatus status;
...
TEST_F(InProcessNodeTests, getRandomOutsByAmountsSuccess) {
  ...
  TycheCash::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response expectedResp;
  TycheCash::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_outs_for_amount out;
  out.amount = 10;
  out.outs.push_back({ 11, ignoredPublicKey });
  expectedResp.outs.push_back(out);
  coreStub.set_random_outs(expectedResp, true);

  std::vector<TycheCash::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_outs_for_amount> outs;
...
TEST_F(InProcessNodeTests, getNewBlocksUninitialized) {
  ...
  std::vector<TycheCash::block_complete_entry> newBlocks;
...
TEST_F(InProcessNodeTests, relayTransactionUninitialized) {
  ...
  CallbackStatus status;
  newNode.relayTransaction(TycheCash::Transaction(), [&] (std::error_code ec) { status.setStatus(ec); });
  ASSERT_TRUE(status.wait());
  ASSERT_NE(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, getBlocksByHeightEmpty) {
  ...
  std::vector<std::vector<TycheCash::BlockDetails>> blocks;
...
TEST_F(InProcessNodeTests, getBlocksByHeightMany) {
  ...
  std::vector<uint32_t> blockHeights;
  std::vector<std::vector<TycheCash::BlockDetails>> actualBlocks;

  std::vector<TycheCash::Block> expectedBlocks;
  ...
  for (auto iter = generator.getBlockchain().begin() + 1; iter != generator.getBlockchain().end(); iter++) {
    ...
    blockHeights.push_back(std::move(boost::get<TycheCash::BaseInput>(iter->baseTransaction.inputs.front()).blockIndex));
  ...
  for (const boost::tuple<boost::tuple<size_t, TycheCash::Block>, std::vector<TycheCash::BlockDetails>>& sameHeight : range) {
    EXPECT_EQ(sameHeight.get<1>().size(), 1);
    for (const TycheCash::BlockDetails& block : sameHeight.get<1>()) {
      EXPECT_EQ(block.height, sameHeight.get<0>().get<0>());
      Crypto::Hash expectedCryptoHash = TycheCash::get_block_hash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getBlocksByHeightFail) {
  ...
  for (const TycheCash::Block& block : generator.getBlockchain()) {
...
TEST_F(InProcessNodeTests, getBlocksByHashEmpty) {
  ...
  std::vector<TycheCash::BlockDetails> blocks;
...
TEST_F(InProcessNodeTests, getBlocksByHashMany) {
  ...
  std::vector<Crypto::Hash> blockHashes;
  std::vector<TycheCash::BlockDetails> actualBlocks;
  ...
  for (auto iter = generator.getBlockchain().begin() + 1; iter != generator.getBlockchain().end(); iter++) {
    expectedBlocks.push_back(*iter);
    blockHashes.push_back(TycheCash::get_block_hash(*iter));
    coreStub.addBlock(*iter);
  }
  ...
  auto range = boost::combine(range1, actualBlocks);
  for (const boost::tuple<boost::tuple<Crypto::Hash, TycheCash::Block>, TycheCash::BlockDetails>& sameHeight : range) {
    Crypto::Hash expectedCryptoHash = TycheCash::get_block_hash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getTxEmpty) {
  ...
  std::vector<TycheCash::TransactionDetails> transactions;
...
TEST_F(InProcessNodeTests, getTxMany) {
  ...
  std::vector<Crypto::Hash> transactionHashes;
  std::vector<TycheCash::TransactionDetails> actualTransactions;

  std::vector<std::tuple<TycheCash::Transaction, Crypto::Hash, uint64_t>> expectedTransactions;

  coreStub.set_blockchain_top(0, boost::value_initialized<Crypto::Hash>());

  size_t prevBlockchainSize = generator.getBlockchain().size();
  for (size_t i = 0; i < BLOCKCHAIN_TX_NUMBER; ++i) {
    auto txptr = TycheCash::createTransaction();
    auto tx = ::createTx(*txptr.get());
    transactionHashes.push_back(TycheCash::getObjectHash(tx));
    ...
    expectedTransactions.push_back(std::make_tuple(tx, TycheCash::get_block_hash(generator.getBlockchain().back()), boost::get<TycheCash::BaseInput>(generator.getBlockchain().back().baseTransaction.inputs.front()).blockIndex));
  }
  ...
  for (const boost::tuple<boost::tuple<Crypto::Hash, TycheCash::TransactionDetails>, std::tuple<TycheCash::Transaction, Crypto::Hash, uint64_t>>& sameHeight : range) {
    Crypto::Hash expectedCryptoHash = TycheCash::getObjectHash(std::get<0>(sameHeight.get<1>()));
    EXPECT_EQ(expectedCryptoHash, sameHeight.get<0>().get<0>());
    Hash expectedHash = reinterpret_cast<const Hash&>(expectedCryptoHash);
...
TEST_F(InProcessNodeTests, getLastLocalBlockTimestamp) {
  ...
  TycheCash::InProcessNode newNode(core, protocolQueryStub);
...
TEST_F(InProcessNodeTests, getPoolDiffereceActualBC) {
  ...
  for (size_t i = 0; i < POOL_TX_NUMBER; ++i) {
    auto txptr = TycheCash::createTransaction();
    auto tx = ::createTx(*txptr.get());
    transactionHashes.insert(TycheCash::getObjectHash(tx));
    TycheCash::tx_verification_context tvc = boost::value_initialized<tx_verification_context>();
  ...
  std::vector<Crypto::Hash> knownPoolTxIds;
  Crypto::Hash knownBlockId = TycheCash::getObjectHash(generator.getBlockchain().back());
  bool isBcActual = false;
  std::vector<std::unique_ptr<ITransactionReader>> newTxs;
  std::vector<Crypto::Hash> deletedTxIds;

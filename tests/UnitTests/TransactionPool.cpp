...
#include "TycheCashCore/TransactionPool.h"
...
class TransactionValidator : public TycheCash::ITransactionValidator {
  virtual bool checkTransactionInputs(const TycheCash::Transaction& tx, BlockInfo& maxUsedBlock) override {
    return true;
  }

  virtual bool checkTransactionInputs(const TycheCash::Transaction& tx, BlockInfo& maxUsedBlock, BlockInfo& lastFailed) override {
    return true;
  }

  virtual bool haveSpentKeyImages(const TycheCash::Transaction& tx) override {
    return false;
  }
...
public:
  ...
  TestTransactionGenerator(const TycheCash::Currency& currency, size_t ringSize) :
    m_currency(currency),
    m_ringSize(ringSize),
    m_miners(ringSize), 
...
namespace
{
  ...
  void GenerateTransaction(const TycheCash::Currency& currency, Transaction& tx, uint64_t fee, size_t outputs) {
  ...
  public:
    ...
    TestPool(const TycheCash::Currency& currency, Logging::ILogger& logger) :
  ...
  public:
    ...
    TycheCash::RealTimeProvider m_time;
...
public:
  TxPool_FillBlockTemplate() :
    tx_pool() {
    currency = TycheCash::CurrencyBuilder(logger).fusionTxMaxSize(TEST_FUSION_TX_MAX_SIZE).blockGrantedFullRewardZone(TEST_MEDIAN_SIZE).currency();
  }

...
TEST(KVSerialize, Simple) {
  ...
  std::string buf = TycheCash::storeToBinaryKeyValue(testData1);
  ASSERT_TRUE(TycheCash::loadFromBinaryKeyValue(testData2, buf));
...
TEST(KVSerialize, BigCollection) {
  ...
  std::string buf = TycheCash::storeToBinaryKeyValue(ts1);
  ASSERT_TRUE(TycheCash::loadFromBinaryKeyValue(ts2, buf));
  EXPECT_EQ(ts1, ts2);
}

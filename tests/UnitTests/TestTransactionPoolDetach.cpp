...
public:
  ...
  void relayTransaction(const TycheCash::Transaction& transaction, const Callback& callback) override {
  ...
  void getPoolSymmetricDifference(std::vector<Crypto::Hash>&& known_pool_tx_ids, Crypto::Hash known_block_id, bool& is_bc_actual, std::vector<std::unique_ptr<TycheCash::ITransactionReader>>& new_txs, std::vector<Crypto::Hash>& deleted_tx_ids, const Callback& callback) override
  {
    std::unique_lock<std::mutex> lk(mutex);
    std::sort(relayedTxs.begin(), relayedTxs.end(), [](const std::pair<uint32_t, TycheCash::Transaction>& val1, const std::pair<uint32_t, TycheCash::Transaction>& val2)->bool {return val1.first < val2.first; });
    is_bc_actual = true;
    ...
    if (detached) {
      ...
      for (; i < relayedTxs.size(); ++i) {
        new_txs.push_back(TycheCash::createTransactionPrefix(relayedTxs[i].second));
      }
    }
  ...
  std::vector<std::pair<uint32_t, TycheCash::Transaction>> relayedTxs;
  uint32_t detachHeight;
  bool detached;
  std::mutex mutex;
};

class WalletSendObserver : public TycheCash::IWalletLegacyObserver
{
public:
  WalletSendObserver() {}
...
public:
  ...
  void generateMoneyForAccount(size_t idx) {
    generator.getBlockRewardForAddress(
      reinterpret_cast<const TycheCash::AccountPublicAddress&>(m_accounts[idx].address));
  }

  std::error_code submitTransaction(ITransactionReader& tx) {
    auto data = tx.getTransactionData();

    TycheCash::BinaryArray txblob(data.data(), data.data() + data.size());
...
struct WaitForExternalTransactionObserver : public TycheCash::IWalletLegacyObserver {
public:

...
class TrivialWalletObserver : public TycheCash::IWalletLegacyObserver
{
public:
...
struct SaveOnInitWalletObserver: public TycheCash::IWalletLegacyObserver {
  SaveOnInitWalletObserver(TycheCash::WalletLegacy* wallet) : wallet(wallet) {
  };
  ...
  TycheCash::WalletLegacy* wallet;
  std::stringstream stream;
};

static const uint64_t TEST_BLOCK_REWARD = 70368744177663;

TycheCash::TransactionId TransferMoney(TycheCash::WalletLegacy& from, TycheCash::WalletLegacy& to, int64_t amount, uint64_t fee, uint64_t mixIn = 0, const std::string& extra = "") {
  TycheCash::WalletLegacyTransfer transfer;
...
{
public:
  WalletLegacyApi() : m_currency(TycheCash::CurrencyBuilder(m_logger).currency()), generator(m_currency) {
  }
...
protected:
  ...
  void GetOneBlockReward(TycheCash::WalletLegacy& wallet);
  void GetOneBlockReward(TycheCash::WalletLegacy& wallet, const TycheCash::Currency& currency, TestBlockchainGenerator& blockchainGenerator);
  void GetOneBlockRewardAndUnlock(TycheCash::WalletLegacy& wallet, TrivialWalletObserver& observer, INodeTrivialRefreshStub& node,
                                  const TycheCash::Currency& currency, TestBlockchainGenerator& blockchainGenerator);
...
void WalletLegacyApi::prepareAliceWallet() {
  ...
  alice.reset(new TycheCash::WalletLegacy(m_currency, *newNode));
  aliceNode = newNode;
...
void WalletLegacyApi::prepareBobWallet() {
  ...
  bob.reset(new TycheCash::WalletLegacy(m_currency, *bobNode));
...
void WalletLegacyApi::prepareCarolWallet() {
  ...
  carol.reset(new TycheCash::WalletLegacy(m_currency, *carolNode));
  carol->addObserver(carolWalletObserver.get());
}

void WalletLegacyApi::GetOneBlockReward(TycheCash::WalletLegacy& wallet) {
  GetOneBlockReward(wallet, m_currency, generator);
}

void WalletLegacyApi::GetOneBlockReward(TycheCash::WalletLegacy& wallet, const TycheCash::Currency& currency, TestBlockchainGenerator& blockchainGenerator) {
...
void WalletLegacyApi::GetOneBlockRewardAndUnlock(TycheCash::WalletLegacy& wallet, TrivialWalletObserver& observer, INodeTrivialRefreshStub& node,
                                                 const TycheCash::Currency& currency, TestBlockchainGenerator& blockchainGenerator) {
...
void WalletLegacyApi::performTransferWithErrorTx(const std::array<int64_t, 5>& amounts, uint64_t fee) {
  std::vector<TycheCash::WalletLegacyTransfer> trs;
  TycheCash::WalletLegacyTransfer tr;
...
TEST_F(WalletLegacyApi, TransactionsAndTransfersAfterSend) {
  ...
  TycheCash::WalletLegacyTransaction tx;
...
TEST_F(WalletLegacyApi, getTransactionSuccess) {
  ...
  EXPECT_EQ(tx.firstTransferId, TycheCash::WALLET_LEGACY_INVALID_TRANSFER_ID);
...
TEST_F(WalletLegacyApi, wrongPassword) {
  ...
  EXPECT_EQ(result.value(), TycheCash::error::WRONG_PASSWORD);
}
...
TEST_F(WalletLegacyApi, sendMoneyToMyself) {
  ...
  TycheCash::TransactionId txId = TransferMoney(*alice, *alice, 100000000, 100);
  ASSERT_NE(txId, TycheCash::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
...
TEST_F(WalletLegacyApi, checkBalanceAfterSend) {
  ...
  TycheCash::TransactionId txId = TransferMoney(*alice, *alice, sendAmount, fee);
  ASSERT_NE(txId, TycheCash::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
...
TEST_F(WalletLegacyApi, moneyInPoolDontAffectActualBalance) {
  ...
  TycheCash::TransactionId txId = TransferMoney(*alice, *bob, sendAmount, fee);
  ASSERT_NE(txId, TycheCash::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
  generator.generateEmptyBlocks(10);
...
TEST_F(WalletLegacyApi, initWithKeys) {
  TycheCash::AccountKeys accountKeys;
...
TEST_F(WalletLegacyApi, sendAfterFailedTransaction) {
  ...
  TycheCash::TransactionId txId = TransferMoney(*alice, *alice, 100000, 100);
  ASSERT_NE(txId, TycheCash::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
  alice->shutdown();
}
...
TEST_F(WalletLegacyApi, outcommingExternalTransactionTotalAmount) {
  class ExternalTxChecker : public TycheCash::IWalletLegacyObserver {
  public:
    ExternalTxChecker(TycheCash::WalletLegacy& wallet) : wallet(wallet), totalAmount(std::numeric_limits<int64_t>::max()) {
    }

    virtual void externalTransactionCreated(TycheCash::TransactionId transactionId) override {
      TycheCash::WalletLegacyTransaction txInfo;
  ...
  TycheCash::WalletLegacy wallet(m_currency, *aliceNode);
...
namespace {
class WalletSynchronizationProgressUpdatedObserver : public TycheCash::IWalletLegacyObserver {
public:
...
TEST_F(WalletLegacyApi, shutdownDoesNotRemoveObservers) {
  ...
  TycheCash::WalletHelper::IWalletRemoveObserverGuard observerGuard(*alice, observer);
...
namespace {
class WalletTransactionEventCounter : public TycheCash::IWalletLegacyObserver {
public:
  virtual void externalTransactionCreated(TycheCash::TransactionId /*transactionId*/) override {
    ++m_count;
  }

  virtual void transactionUpdated(TycheCash::TransactionId /*transactionId*/) override {
    ++m_count;
  }
...
TEST_F(WalletLegacyApi, afterShutdownAndInitWalletDoesNotSendNotificationsRelatedToOldAddress) {
  ...
  TycheCash::AccountKeys accountKeys1;
  ...
  std::vector<TycheCash::WalletLegacyTransfer> transfers;
...
TEST_F(WalletLegacyApi, resetDoesNotChangeAccountKeys) {
  ...
  TycheCash::AccountKeys expectedAccountKeys;
  alice->getAccountKeys(expectedAccountKeys);

  alice->reset();

  TycheCash::AccountKeys actualAccountKeys;
...
void generateWallet(TycheCash::IWalletLegacy& wallet, TrivialWalletObserver& observer, const std::string& pass) {
...
TEST_F(WalletLegacyApi, outdatedUnconfirmedTransactionDeletedOnNewBlock) {
  ...
  TycheCash::Currency currency(TycheCash::CurrencyBuilder(m_logger).mempoolTxLiveTime(TRANSACTION_MEMPOOL_TIME).currency());
  TestBlockchainGenerator blockchainGenerator(currency);
  INodeTrivialRefreshStub node(blockchainGenerator);
  TycheCash::WalletLegacy wallet(currency, node);
  TrivialWalletObserver walletObserver;
  wallet.addObserver(&walletObserver);
  ...
  TycheCash::WalletLegacyTransaction transaction;
  ASSERT_TRUE(wallet.getTransaction(id, transaction));
  EXPECT_EQ(TycheCash::WalletLegacyTransactionState::Deleted, transaction.state);
...
TEST_F(WalletLegacyApi, walletLoadsNullSpendSecretKey) {
  ...
  Crypto::generate_keys(accountKeys.address.spendPublicKey, accountKeys.spendSecretKey);
  Crypto::generate_keys(accountKeys.address.viewPublicKey, accountKeys.viewSecretKey);
  accountKeys.spendSecretKey = TycheCash::NULL_SECRET_KEY;

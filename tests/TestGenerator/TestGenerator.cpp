...
uint64_t test_generator::getAlreadyGeneratedCoins(const TycheCash::Block& blk) const {
  Crypto::Hash blkHash;
  get_block_hash(blk, blkHash);
  return getAlreadyGeneratedCoins(blkHash);
}

void test_generator::addBlock(const TycheCash::Block& blk, size_t tsxSize, uint64_t fee,
...
bool test_generator::constructBlock(TycheCash::Block& blk, uint32_t height, const Crypto::Hash& previousBlockHash,
...
bool test_generator::constructBlock(TycheCash::Block& blk, const TycheCash::AccountBase& minerAcc, uint64_t timestamp) {
  std::vector<size_t> blockSizes;
  std::list<TycheCash::Transaction> txList;
  return constructBlock(blk, 0, NULL_HASH, minerAcc, timestamp, 0, blockSizes, txList);
}

bool test_generator::constructBlock(TycheCash::Block& blk, const TycheCash::Block& blkPrev,
                                    const TycheCash::AccountBase& minerAcc,
...
bool test_generator::constructBlockManuallyTx(TycheCash::Block& blk, const TycheCash::Block& prevBlock,
                                              const TycheCash::AccountBase& minerAcc,
...
bool test_generator::constructMaxSizeBlock(TycheCash::Block& blk, const TycheCash::Block& blkPrev,
                                           const TycheCash::AccountBase& minerAccount,
...
void fillNonce(TycheCash::Block& blk, const difficulty_type& diffic) {
  blk.nonce = 0;
  Crypto::cn_context context;
...
bool constructMinerTxBySize(const TycheCash::Currency& currency, TycheCash::Transaction& baseTransaction, uint32_t height,
  ...
  if (!currency.constructMinerTx(height, Common::medianValue(blockSizes), alreadyGeneratedCoins, targetBlockSize,
      fee, minerAddress, baseTransaction, TycheCash::BinaryArray(), 1)) {
    return false;
  }

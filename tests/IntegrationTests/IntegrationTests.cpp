...
public:
  ...
  IntegrationTest() : 
    currency(TycheCash::CurrencyBuilder(log).testnet(true).currency()), 
  ...
  void makeWallets() {
    for (auto& n: inodes) {
      std::unique_ptr<TycheCash::IWalletLegacy> wallet(new TycheCash::WalletLegacy(m_currency, *n));
...
TEST_F(IntegrationTest, BlockPropagationSpeed) {
  ...
  {
    std::unique_ptr<TycheCash::INode>& localNode = inodes.front();
    std::unique_ptr<TycheCash::INode>& remoteNode = inodes.back();

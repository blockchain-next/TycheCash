...
public:
  ...
  TycheCash::TransactionId waitExternalTransaction() {
    std::unique_lock<std::mutex> lk(m_mutex);
    ...
    TycheCash::TransactionId txId = m_externalTransactions.front();
  ...
  std::error_code waitSendResult(TycheCash::TransactionId txid) {
    std::unique_lock<std::mutex> lk(m_mutex);

    std::unordered_map<TycheCash::TransactionId, std::error_code>::iterator it;
...
private:
  ...
  std::unordered_map<TycheCash::TransactionId, std::error_code> m_sendResults;
  std::deque<TycheCash::TransactionId> m_externalTransactions;
};

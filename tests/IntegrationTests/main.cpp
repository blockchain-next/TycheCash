...
public:
  ...
  SimpleTest(const TycheCash::Currency& currency, System::Dispatcher& system, const Tests::Common::BaseFunctionalTestsConfig& config) : 
    BaseFunctionalTests(currency, system, config) {}

  class WaitForActualGrowObserver : public TycheCash::IWalletLegacyObserver {
  ...
  class WaitForActualDwindleObserver : public TycheCash::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForPendingGrowObserver : public TycheCash::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForConfirmationObserver : public TycheCash::IWalletLegacyObserver {
  ...
  class WaitForSendCompletedObserver : public TycheCash::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_Sent;
    std::error_code& m_error;
    TycheCash::TransactionId& m_transactionId;

  public:
    WaitForSendCompletedObserver(Tests::Common::Semaphore& Sent, TycheCash::TransactionId& transactionId, std::error_code& error) : m_Sent(Sent), m_transactionId(transactionId), m_error(error) { }
  ...
  class WaitForExternalTransactionObserver : public TycheCash::IWalletLegacyObserver {
  public:
    WaitForExternalTransactionObserver() { }
    std::promise<TycheCash::TransactionId> promise;
  ...
  class WaitForTransactionUpdated : public TycheCash::IWalletLegacyObserver {
  public:
    ...
    TycheCash::TransactionId expectindTxId;
  };
  ...
  bool perform1() {
    ...
    std::unique_ptr<TycheCash::INode> node1;
    std::unique_ptr<TycheCash::INode> node2;

    nodeDaemons.front()->makeINode(node1);
    nodeDaemons.front()->makeINode(node2);

    std::unique_ptr<TycheCash::IWalletLegacy> wallet1;
    std::unique_ptr<TycheCash::IWalletLegacy> wallet2;
  ...
  class WaitForBlockchainHeightChangeObserver : public TycheCash::INodeObserver {
  ...
  class CallbackHeightChangeObserver : public TycheCash::INodeObserver {
  ...
  bool perform2(size_t blocksCount = 10)
  {
    ...
    mineBlock();
    LOG_TRACE("STEP 2 PASSED");
    std::unique_ptr<TycheCash::INode> localNode;
    std::unique_ptr<TycheCash::INode> remoteNode;

    nodeDaemons.front()->makeINode(localNode);
    nodeDaemons.back()->makeINode(remoteNode);
    
    std::unique_ptr<TycheCash::IWalletLegacy> wallet;
  ...
  bool perform4() {
    ...
    std::unique_ptr<TycheCash::INode> hopNode;
    std::unique_ptr<TycheCash::INode> localNode;
    std::unique_ptr<TycheCash::INode> remoteNode;
  ...
  bool perform5() {
    ...
    std::unique_ptr<TycheCash::INode> node1;
    std::unique_ptr<TycheCash::INode> inprocNode;
...
void testMultiVersion(const TycheCash::Currency& currency, System::Dispatcher& d, const Tests::Common::BaseFunctionalTestsConfig& config);
...
TEST_F(SimpleTestCase, TESTPOOLDELETION) {
  currency = TycheCash::CurrencyBuilder(logger).testnet(true).mempoolTxLiveTime(60).currency();

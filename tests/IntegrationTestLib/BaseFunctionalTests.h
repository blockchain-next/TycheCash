...
namespace Tests {
  ...
  namespace po = boost::program_options;
    ...
    public:
      BaseFunctionalTests(const TycheCash::Currency& currency, System::Dispatcher& d, const BaseFunctionalTestsConfig& config) :
          m_dispatcher(d),
          m_currency(currency),
    ...
    protected:
      ...
      bool makeWallet(std::unique_ptr<TycheCash::IWalletLegacy> & wallet, std::unique_ptr<TycheCash::INode>& node, const std::string& password = "pass");
      bool mineBlocks(TestNode& node, const TycheCash::AccountPublicAddress& address, size_t blockCount);
      bool mineBlock(std::unique_ptr<TycheCash::IWalletLegacy>& wallet);
      bool mineBlock();
      bool startMining(size_t threads);
      bool stopMining();

      bool getNodeTransactionPool(size_t nodeIndex, TycheCash::INode& node, std::vector<std::unique_ptr<TycheCash::ITransactionReader>>& txPool);

      bool waitDaemonsReady();
      bool waitDaemonReady(size_t nodeIndex);
      bool waitForPeerCount(TycheCash::INode& node, size_t expectedPeerCount);
      bool waitForPoolSize(size_t nodeIndex, TycheCash::INode& node, size_t expectedPoolSize,
        std::vector<std::unique_ptr<TycheCash::ITransactionReader>>& txPool);

      bool prepareAndSubmitBlock(TestNode& node, TycheCash::Block&& blockTemplate);
    ...
    private:
      ...
      std::unique_ptr<TycheCash::INode> mainNode;
      std::unique_ptr<TycheCash::IWalletLegacy> workingWallet;

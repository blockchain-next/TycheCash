...
#ifdef _WIN32
const std::string DAEMON_FILENAME = std::string(TycheCash_NAME) + "d.exe";
#else
const std::string DAEMON_FILENAME = std::string(TycheCash_NAME) + "d";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_" << TycheCash_NAME << "d_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"" + std::string(TycheCash_NAME) + "d" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), (std::string(TycheCash_NAME) + "d").c_str(), "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
...
namespace {
  class WaitForCoinBaseObserver : public TycheCash::IWalletLegacyObserver {
    Semaphore& m_gotReward;
    TycheCash::IWalletLegacy& m_wallet;
  public:
    WaitForCoinBaseObserver(Semaphore& gotReward, TycheCash::IWalletLegacy& wallet) : m_gotReward(gotReward), m_wallet(wallet) { }
    virtual void externalTransactionCreated(TycheCash::TransactionId transactionId) override {
      TycheCash::WalletLegacyTransaction trInfo;
...
bool BaseFunctionalTests::mineBlocks(TestNode& node, const TycheCash::AccountPublicAddress& address, size_t blockCount) {
...
bool BaseFunctionalTests::prepareAndSubmitBlock(TestNode& node, TycheCash::Block&& blockTemplate) {
  blockTemplate.timestamp = m_nextTimestamp;
  m_nextTimestamp += 2 * m_currency.difficultyTarget();

  BinaryArray blockBlob = TycheCash::toBinaryArray(blockTemplate);
  return node.submitBlock(::Common::toHex(blockBlob.data(), blockBlob.size()));
}

bool BaseFunctionalTests::mineBlock(std::unique_ptr<TycheCash::IWalletLegacy> &wallet) {
...
bool BaseFunctionalTests::makeWallet(std::unique_ptr<TycheCash::IWalletLegacy> & wallet, std::unique_ptr<TycheCash::INode>& node, const std::string& password) {
  if (!node) return false;
  wallet = std::unique_ptr<TycheCash::IWalletLegacy>(new TycheCash::WalletLegacy(m_currency, *node));
...
namespace {
  struct PeerCountWaiter : TycheCash::INodeObserver {
...
bool BaseFunctionalTests::waitForPeerCount(TycheCash::INode& node, size_t expectedPeerCount) {
...
bool BaseFunctionalTests::waitForPoolSize(size_t nodeIndex, TycheCash::INode& node, size_t expectedPoolSize,
  std::vector<std::unique_ptr<TycheCash::ITransactionReader>>& txPool) {
...
bool BaseFunctionalTests::getNodeTransactionPool(size_t nodeIndex, TycheCash::INode& node,
  std::vector<std::unique_ptr<TycheCash::ITransactionReader>>& txPool) {

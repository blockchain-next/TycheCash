...
bool RPCTestNode::getBlockTemplate(const std::string& minerAddress, TycheCash::Block& blockTemplate, uint64_t& difficulty) {
...
bool RPCTestNode::makeINode(std::unique_ptr<TycheCash::INode>& node) {
  std::unique_ptr<TycheCash::INode> newNode(new TycheCash::NodeRpcProxy("127.0.0.1", m_rpcPort));
  NodeCallback cb;
...
uint64_t RPCTestNode::getLocalHeight() {
  try {
    TycheCash::COMMAND_RPC_GET_INFO::request req;
    TycheCash::COMMAND_RPC_GET_INFO::response rsp;
